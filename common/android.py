...
def get_imei(slot):
    ...
    ret = parse_service_call_string(service_call(
        ["iphonesubinfo", "3", "i32", str(slot)]))
    if not ret:
...
def service_call(call):
    ...
    ret = subprocess.check_output(
        ["service", "call", *call], encoding='utf8').strip()
...
def get_network_type():
    ...
    if wifi_check is None:
        ...
        cell_check = parse_service_call_unpack(
            service_call(['phone', '59']), ">q")
