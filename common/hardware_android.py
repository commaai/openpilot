...
class Android(HardwareBase):
  ...
  def get_battery_capacity(self):
    return self.read_param("/sys/class/power_supply/battery/capacity", int, 100)
  ...
  def get_battery_status(self):
    ...
    return self.read_param("/sys/class/power_supply/battery/status", lambda x: x.strip(), '')

  def get_battery_current(self):
    return self.read_param("/sys/class/power_supply/battery/current_now", int)

  def get_battery_voltage(self):
    return self.read_param("/sys/class/power_supply/battery/voltage_now", int)

  def get_battery_charging(self):
    # This does correspond with actually charging
    return self.read_param("/sys/class/power_supply/battery/charge_type", lambda x: x.strip() != "N/A", True)
