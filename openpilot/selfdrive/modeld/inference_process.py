...
class InferenceProcess:
  """Main-thread supervisor with one in-flight request and a deadline independent of inference and IPC payload writes."""
  def __init__(self, factory, args=(), *, startup_timeout=60.0, cleanup=None):
    ...
    try:
      # Register ownership and caller cleanup before delivering SIGINT.
      with _defer_sigint():
        self.process.start()
  ...
  def _receive(self, timeout):
    ...
    if not self.connection.poll(max(0., deadline - time.monotonic())) or time.monotonic() > deadline:
