...
if __name__ == "__main__":
  ...
  input_specs = {name: (spec.shape, spec.dtype) for name, spec in model_runner.graph_inputs.items()}
  state_pairs = {name: f'next_{name}' for name in input_specs if f'next_{name}' in model_runner.graph_outputs}
  out = {
  ...
  for cam_w, cam_h in args.camera_resolutions:
    ...
    make_model_queues = partial(make_input_queues, input_specs, state_pairs,
                                frame_copy_size=frame_copy_size)
    warp = make_warp(nv12, model_w, model_h)
    run_model_jit = TinyJit(make_run_model(warp, model_runner, input_specs, state_pairs, frame_copy_size), prune=True)
