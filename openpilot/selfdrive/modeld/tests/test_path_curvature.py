...
class TestPathCurvature(unittest.TestCase):
  def test_straight(self):
    self.assertEqual(curvature_from_path(np.array([[0, 0], [2, 0], [5, 0]]), 5.0), 0.0)
  ...
  def test_interpolates_by_distance_along_path(self):
    ...
    self.assertAlmostEqual(curvature_from_path(path, 5.0), 6 / 13)
  ...
  def test_quarter_circle_both_directions(self):
    ...
    for sign in (-1, 1):
      ...
      self.assertAlmostEqual(curvature_from_path(path, 5.0), sign / 4, places=5)

  def test_duplicate_points(self):
    path = np.array([[0, 0], [0, 0], [1, 0], [1, 0], [4, 0]])
    self.assertEqual(curvature_from_path(path, 5.0), 0.0)

  def test_short_paths_use_endpoint(self):
    for x, y in [(2.9, 0), (2, 1), (2, -1), (0.2, 0.01)]:
      path = np.array([[0, 0], [x, y], [x, y]])
      self.assertAlmostEqual(curvature_from_path(path, 5.0), 2 * y / (x * x + y * y))
  ...
  def test_five_meter_boundary_is_continuous(self):
    ...
    for length in [4.999, 5., 5.001]:
      ...
      values.append(curvature_from_path(np.array([[0, 0], endpoint]), 5.0))
  ...
  def test_stationary_path_falls_back(self):
    self.assertIsNone(curvature_from_path(np.zeros((33, 3)), 5.0))
  ...
  def test_invalid_paths_fall_back(self):
    for path in (np.array([]), np.array([[0, 0]]), np.zeros((3, 1)),
      ...
      self.assertIsNone(curvature_from_path(path, 5.0))

  def test_target_behind_car_falls_back(self):
    self.assertIsNone(curvature_from_path(np.array([[0, 0], [-4, 0]]), 5.0))
