...
int PandaSpiHandle::spi_transfer_retry(uint8_t endpoint, uint8_t *tx_data, uint16_t tx_len, uint8_t *rx_data, uint16_t max_rx_len, unsigned int timeout, const uint64_t *write_generation) {
  ...
  std::lock_guard operation_lock(hw_lock);
  int ret;
  int nack_count = 0;
  int timeout_count = 0;
...
int PandaSpiHandle::wait_for_ack(uint8_t ack, uint8_t tx, unsigned int timeout, unsigned int length) {
  ...
  while (true) {
    ...
    if (rx_buf[0] == ack) {
      ...
      LOGD("SPI: got NACK, waiting for 0x%x", ack);
      return SpiError::NACK;
    }

    // handle timeout
    if (millis_since_boot() - start_millis > timeout) {
      LOGW("SPI: timed out waiting for ACK, waiting for 0x%x", ack);
...
static void wait_for_v3_boundary(uint64_t start_ns, bool after_request, bool maintenance = false, int can_records = 0, bool can_read = false) {
  ...
  constexpr uint64_t request_gap = 500 * 1000ULL;
  constexpr uint64_t recovery_gap = 250 * 1000ULL;
  constexpr uint64_t maintenance_gap = 100000 * 1000ULL;
  constexpr uint64_t can_record_gap = 4 * 1000ULL;
  constexpr uint64_t can_read_gap = 750 * 1000ULL;
...
int PandaSpiHandle::spi_transfer_v3(uint8_t endpoint, const uint8_t *tx_data, uint16_t tx_len,
  ...
  const bool maintenance = endpoint == 2 || (endpoint == 0 && tx_len > 0 && tx_data[0] == 0xb5);
  ...
  auto encode_request = [&]() {
    ...
    if (endpoint == 3) max_rx_len = can_rx_pending_size == 0 ? SPI_PROTO_MAX_PAYLOAD : 0;
  ...
  do {
    ...
    spi_ioc_transfer request = {.tx_buf = (uint64_t)request_bytes.data(), .len = request_size};
    ...
    if (reply.endpoint == SPI_PROTO_SESSION || reply.endpoint == SPI_PROTO_SEQUENCE) {
      LOGE("SPI v3: session or sequence lost, outcome unknown");
      connected = comms_healthy = false;
      return -1;
    }
  ...
  LOGE("SPI v3: deadline expired, ending session%s", outcome_unknown ? " (outcome unknown)" : "");
  connected = comms_healthy = false;
  return -1;
}
