...
class TestCalibrationd(OpenpilotTestCase):
  ...
  def test_read_saved_params(self):
    ...
    msg.cameraCalibration.validBlocks = random.randint(1, 10)
    msg.cameraCalibration.rpyCalib = [random.random() for _ in range(3)]
    msg.cameraCalibration.height = [random.random() for _ in range(1)]
    Params().put("CalibrationParams", msg.to_bytes(), block=True)
    c = Calibrator(param_put=True)

    np.testing.assert_allclose(msg.cameraCalibration.rpyCalib, c.rpy)
    np.testing.assert_allclose(msg.cameraCalibration.height, c.height)
    assert msg.cameraCalibration.validBlocks == c.valid_blocks
  ...
  def test_calibration_auto_reset(self):
    ...
    assert c.cal_status == log.CameraCalibration.Status.calibrated
    ...
    assert c.cal_status == log.CameraCalibration.Status.recalibrating
