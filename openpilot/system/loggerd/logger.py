...
def build_init_data() -> bytes:
  ...
  known_param_keys = {key.decode() for key in params.all_keys()}
  ...
  for entry, (key, value) in zip(param_entries, sorted(params_map.items()), strict=True):
    entry.key = key
    entry.value = b"" if key in known_param_keys and params.get_flags(key) & ParamKeyFlag.DONT_LOG else value
