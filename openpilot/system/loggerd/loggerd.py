...
class Loggerd:
  def __init__(self):
    ...
    for name, service in SERVICE_LIST.items():
      ...
      sock = messaging.sub_sock(name, poller=self.poller, addr="127.0.0.1", conflate=False)
      self.service_states[sock] = ServiceState(
        name=name,
        counter=0,
      ...
      if encoder:
        self.remote_encoders[sock] = RemoteEncoder()

    self.encoders_with_audio = [
      self.remote_encoders[sock]
      for sock, service in self.service_states.items()
  ...
  def _write_encode_data(self, event, remote: RemoteEncoder, encoder_info: EncoderInfo) -> int:
    ...
    setattr(idx_msg, encoder_info.idx_name, idx.to_dict())
        ...
        for sock, service in self.service_states.items():
          ...
          while not self.exit_handler.exit:
            ...
            if service.encoder:
              ...
              bytes_count += self.handle_encoder_message(data, service.name, self.remote_encoders[sock], self.encoder_infos[service.name])
            else:
