...
def _read_files(path: str | Path) -> dict[str, bytes]:
  try:
    ...
    return {}
  return {entry.name: safe.read_file(entry, b"") for entry in entries if not entry.is_dir()}
...
def build_boot_log() -> bytes:
  ...
  command_entry.value = safe.check_output(command, b"", shell=True)

  boot.launchLog = safe.read_file("/tmp/launch_log", b"").decode("utf-8", "replace")
  return msg.to_bytes()
