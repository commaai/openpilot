...
TEST_TIMESPAN = 10
CAMERAS = ('narrowRoadCameraState', 'driverCameraState', 'wideRoadCameraState')
...
def _camera_session():
  ...
  with processes_context(["camerad"]), log_collector(CAMERAS) as (raw_logs, lock):
    ...
    while time.monotonic() - start < MAX_TEST_TIME:
      rpic, dpic = get_snapshots(frame="narrowRoadCameraState", front_frame="driverCameraState")
...
class TestCamerad(OpenpilotTestCase):
  ...
  def test_frame_sync(self):
    ...
    for i in n:
      offset_ms = abs(self.logs['driverCameraState']['timestampSof'][i] - self.logs['narrowRoadCameraState']['timestampSof'][i]) / 1e6
