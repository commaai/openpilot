...
from cereal.messaging.tests.test_messaging import (
  assert_carstate,
  events,
  random_bytes,
  random_carstate,
...
class TestSubMaster:
    ...
    def test_init(self):
        ...
        for p in [sm.updated, sm.recv_time, sm.recv_frame, sm.alive, sm.sock, sm.data, sm.logMonoTime, sm.valid]:
    ...
    def test_init_state(self):
        ...
        assert all(t == 0.0 for t in sm.recv_time.values())
    ...
    def test_avg_frequency_checks(self):
        for poll in (True, False):
            sm = messaging.SubMaster(
                ["modelV2", "carParams", "carState", "cameraOdometry", "liveCalibration"],
                poll=("modelV2" if poll else None),
                frequency=(20.0 if not poll else None),
            )
