...
import capnp
import msgq
from msgq import drain_sock_raw, pub_sock, sub_sock
...
def log_from_bytes(
    dat: bytes, struct: capnp.lib.capnp._StructModule = log.Event
...
def new_message(service: Optional[str], size: Optional[int] = None, **kwargs) -> capnp.lib.capnp._DynamicStructBuilder:
    args = {"valid": False, "logMonoTime": int(time.monotonic() * 1e9), **kwargs}
...
class FrequencyTracker:
    def __init__(self, service_freq: float, update_freq: float, is_poll: bool):
        freq = max(min(service_freq, update_freq), 1.0)
        if is_poll:
...
class SubMaster:
    def __init__(
        ...
        assert (
            frequency is None or poll is None
        ), "Do not specify 'frequency' - frequency of the polled service will be used."
    ...
    def update_msgs(self, cur_time: float, msgs: List[capnp.lib.capnp._DynamicStructReader]) -> None:
        ...
        for s in self.static_freq_services:
            ...
            self.alive[s] = (cur_time - self.recv_time[s]) < (10.0 / SERVICE_LIST[s].frequency) or (
                self.seen[s] and self.simulation
            )
...
class PubMaster:
    ...
    def wait_for_readers_to_update(self, s: str, timeout: int, dt: float = 0.05) -> bool:
        for _ in range(int(timeout * (1.0 / dt))):
