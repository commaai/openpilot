...
class _Qr:
  ...
  def _format(self, mask: int) -> None:
    data = 0x08 | mask  # level L is format value 01
    rem = data
    for _ in range(10):
    ...
    for i in range(15):
      bit = ((value >> i) & 1) != 0
      a = i if i < 6 else i + 1 if i < 8 else self.size - 15 + i
      self._set_function(8, a, bit)
      b = self.size - 1 - i if i < 8 else 15 - i if i < 9 else 14 - i
      self._set_function(b, 8, bit)
    ...
    while right >= 1:
      ...
      for vert in range(self.size):
        ...
        for x in (right, right - 1):
          if not self.function[y][x] and bit < len(data) * 8:
...
def make_texture(data: str, inverted: bool = False) -> rl.Texture:
  ...
  for version in range(1, 21):
    ...
    if len(raw) < (1 << count_bits) and 4 + count_bits + len(raw) * 8 <= _capacity(version) * 8:
      break
  else:
