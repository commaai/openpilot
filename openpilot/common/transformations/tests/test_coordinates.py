...
geodetic_positions_deg = np.array(
    [
        [37.7610403, -122.4778699, 115],
...
ecef_positions = np.array(
    [
        [-2711076.55270557, -4259167.14692758, 3884579.87669935],
...
ecef_positions_offset = np.array(
    [
        [-2711004.46961115, -4259099.33540613, 3884605.16002147],
...
ned_offsets = np.array(
    [
        [78.722153649976391, 24.396208657446344, 60.343017506838436],
...
ecef_positions_offset_batch = np.array(
    [
        [2068089.41454771, -5273434.46829148, 2927074.04783672],
...
ned_offsets_batch = np.array(
    [
        [53.88103168, 43.83445935, -46.27488057],
...
class TestNED:
    ...
    def test_ecef_geodetic(self):
        ...
        np.testing.assert_allclose(
            geodetic_positions[0, 2], coord.ecef2geodetic(ecef_positions[0])[2], rtol=1e-9, atol=1e-4
        )

        np.testing.assert_allclose(geodetic_positions[:, :2], coord.ecef2geodetic(ecef_positions)[:, :2], rtol=1e-9)
        np.testing.assert_allclose(
            geodetic_positions[:, 2], coord.ecef2geodetic(ecef_positions)[:, 2], rtol=1e-9, atol=1e-4
        )
    ...
    def test_ned(self):
        ...
        for geo_pos in geodetic_positions:
            ...
            geo_pos_double_converted_moved = converter.ned2geodetic(
                converter.geodetic2ned(geo_pos) + np.array([0, 0, -10])
            )
    ...
    def test_ned_saved_results(self):
        for i, ecef_pos in enumerate(ecef_positions):
            converter = coord.LocalCoord.from_ecef(ecef_pos)
            np.testing.assert_allclose(
                converter.ned2ecef(ned_offsets[i]), ecef_positions_offset[i], rtol=1e-9, atol=1e-4
            )
            np.testing.assert_allclose(
                converter.ecef2ned(ecef_positions_offset[i]), ned_offsets[i], rtol=1e-9, atol=1e-4
            )

    def test_ned_batch(self):
        converter = coord.LocalCoord.from_ecef(ecef_init_batch)
        np.testing.assert_allclose(
            converter.ecef2ned(ecef_positions_offset_batch), ned_offsets_batch, rtol=1e-9, atol=1e-7
        )
        np.testing.assert_allclose(
            converter.ned2ecef(ned_offsets_batch), ecef_positions_offset_batch, rtol=1e-9, atol=1e-7
        )
