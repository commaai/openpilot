...
bool ReplayStream::loadRoute(const std::string &route, const std::string &data_dir, uint32_t replay_flags, bool auto_source) {
  replay.reset(new Replay(route, {"can", "narrowRoadEncodeIdx", "driverEncodeIdx", "wideRoadEncodeIdx", "carParams"},
...
AbstractStream *OpenReplayWidget::open() {
  ...
  if (!is_valid_format) {
    ...
    if (cameras[1]->isChecked()) flags |= REPLAY_FLAG_DRIVER_CAMERA;
