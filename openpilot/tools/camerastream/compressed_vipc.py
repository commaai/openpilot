...
from openpilot.tools.lib.libav import VideoDecoder
...
def decoder(addr, vipc_server, vst, debug=False):
  sock_name = ENCODE_SOCKETS[vst]
  if debug:
    print(f"start decoder for {sock_name}")
  ...
  time_q = []
  with VideoDecoder("hevc") as codec:
    while 1:
      ...
      for evt in msgs:
        ...
        # prepend the stream header (VPS/SPS/PPS) to the first packet
        packet = bytes(evta.data)
        if not seen_iframe:
          packet = bytes(evta.header) + packet
          seen_iframe = True
        ...
        if len(frames) == 0:
          ...
          continue
        assert len(frames) == 1
        vipc_server.send(vst, frames[0].reshape(-1).data, cnt, int(time_q[0]*1e9), int(time.monotonic()*1e9))
        cnt += 1
...
class CompressedVipc:
  def __init__(self, addr, vision_streams, server_name, debug=False):
    ...
    self.procs = []
    for vst in vision_streams:
      p = multiprocessing.Process(target=decoder, args=(addr, self.vipc_server, vst, debug))
      p.start()
      self.procs.append(p)
