...
void draw_plot_pane(Session &session, PaneInstance &pane) {
  ...
  if (ImPlot::BeginPlot("##loggy_plot", ImGui::GetContentRegionAvail(), plot_flags)) {
    ...
    for (size_t i = 0; i < series.size(); ++i) {
      ...
      ImPlotSpec spec;
      spec.LineColor = kPlotColors[i % std::size(kPlotColors)];
      spec.LineWeight = item.stairs ? 1.8f : 2.0f;
      if (item.stairs) {
        spec.Flags = ImPlotStairsFlags_PreStep;
      } else {
        spec.Flags = ImPlotLineFlags_SkipNaN;
      }
      const std::string label = legend_label(item, label_width) + "###" + item.request.path;
      if (item.stairs) {
