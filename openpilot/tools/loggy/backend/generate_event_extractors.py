...
def deprecated_path(path):
  return "/deprecated/" in path or "DEPRECATED" in path
...
class Generator:
  ...
  def emit_field(self, indent, struct_schema, reader_expr, field_name, base_path, base_path_expr, dynamic_path):
    ...
    if dynamic_path or deprecated_path(field_path):
      field_path_var = self.tmp("path")
      if dynamic_path:
        self.emit(indent, f"const std::string {field_path_var} = {base_path_expr} + {cxx_string('/' + field_name)};")
      else:
        self.emit(indent, f"const std::string {field_path_var} = {cxx_string(field_path)};")
  ...
  def emit_list(self, indent, type_proto, schema, list_expr, path, path_expr, dynamic_path):
    ...
    if elem_scalar is not None:
      ...
      self.emit(indent + 2, f"for (size_t {index_var} = 0; {index_var} < {list_expr}.size(); ++{index_var}) {{")
    ...
    if elem_kind in {"struct", "list"}:
      index_var = self.tmp("i")
      self.emit(indent, f"for (size_t {index_var} = 0; {index_var} < {list_expr}.size(); ++{index_var}) {{")
  ...
  def emit_can_special(self, indent, service_name):
    ...
    self.emit(indent, "if (options.include_raw_can_events) {")
    self.emit(indent + 2, f"const CanServiceKind can_service = {service_kind};")
    self.emit(indent + 2, f"for (const auto &msg : event.{accessor('get', service_name)}()) {{")
    self.emit(indent + 4, "append_can_frame(can_service, static_cast<uint8_t>(msg.getSrc()), msg.getAddress(),")
    self.emit(indent + 4, "                 msg.getDeprecated().getBusTime(), msg.getDat(), tm, series);")
    self.emit(indent + 2, "}")
    self.emit(indent, "}")
    self.emit(indent, "if (!options.include_can_scalar_fields) {")
  ...
  def generate(self):
    self.lines = []
    self.emit(0, "// Generated by openpilot/tools/loggy/backend/generate_event_extractors.py; do not edit.")
    self.emit(0, "")
    ...
    self.emit(0, "}")
    self.emit(0, "")
    self.emit(0, "bool append_event_static_reader(cereal::Event::Which which, const cereal::Event::Reader &event,")
    self.emit(0, "                                const SegmentExtractOptions &options, SeriesAccumulator *series) {")
...
if __name__ == "__main__":
  ...
  log = capnp.load(
    str(repo_root / "openpilot" / "cereal" / "log.capnp"),
