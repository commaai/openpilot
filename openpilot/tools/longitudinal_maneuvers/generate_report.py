...
def report(platform, route, _description, CP, ID, maneuvers):
  ...
  for description, runs in maneuvers:
    ...
    for run, msgs in enumerate(runs):
      ...
      t_devicePose, devicePose = zip(*[(m.logMonoTime, m.devicePose) for m in msgs if m.which() == 'devicePose'], strict=True)
      ...
      t_devicePose = [(t - t_devicePose[0]) / 1e9 for t in t_devicePose]
      ...
      prev_crossed = False
      for t, lp in zip(t_devicePose, devicePose, strict=True):
      ...
      ax[0].plot(t_devicePose, [m.accelerationDevice.x for m in devicePose], label='devicePose.accelerationDevice.x', linewidth=6)
