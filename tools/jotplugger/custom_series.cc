#include "tools/jotplugger/app.h"
#include "tools/jotplugger/common.h"
...
fs::path create_custom_series_temp_dir() {
  ...
  const fs::path dir = fs::temp_directory_path() / ("jotplugger_math_" + std::to_string(::getpid()) + "_" + std::to_string(stamp));
...
PythonEvalResult evaluate_custom_python_series(const AppSession &session,
  ...
  try {
    ...
    const CommandResult process = run_process_capture_output({
      "python3",
      (repo_root() / "tools" / "jotplugger" / "math_eval.py").string(),
