#!/usr/bin/env python3
"""Fuzz test for jotplugger — random UI interactions looking for crashes and freezes.
...
BINARY = Path(__file__).resolve().parent / "jotplugger"
...
class Session:
  ...
  def start(self, *, route: str, layout: str | None = None, sync_load: bool = False):
    ...
    proc = subprocess.run(
      ["xdotool", "search", "--sync", "--name", "jotplugger"],
      env=self.env,
      capture_output=True,
      text=True,
      timeout=30,
    ...
    assert wids, "jotplugger window never appeared"
    self.winid = wids[0]
  ...
  def screenshot(self, path: Path) -> float:
    ...
    try:
      subprocess.run(["xwd", "-silent", "-id", self.winid, "-out", str(xwd)], env=self.env, check=True, timeout=FREEZE_TIMEOUT_S)
      subprocess.run(["convert", str(xwd), str(path)], env=self.env, check=True, timeout=FREEZE_TIMEOUT_S)
    finally:
...
def _make_fuzz_params():
  return [pytest.param(s, r, id=f"{s.name}-r{r:02d}") for s in SCENARIOS for r in range(ROUNDS_PER_SCENARIO)]
...
def run_layout_screenshot(layout: str):
  ...
  try:
    ...
    assert session.alive, f"jotplugger crashed on startup: {session.crash_reason()}"
    ...
    prompt_parts = [
      f"You are reviewing the '{layout}' layout in jotplugger, a vehicle telemetry plotting tool.",
    ...
    prompt_parts.append("Reply with exactly PASS or FAIL on the first line, then a one-sentence explanation.")
    prompt = "\n".join(prompt_parts)
...
@pytest.mark.skipif(not BINARY.exists(), reason="jotplugger binary not built")
class TestJotpluggerFuzz:
