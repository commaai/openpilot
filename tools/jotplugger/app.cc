...
fs::path layouts_dir() {
  return repo_root() / "tools" / "jotplugger" / "layouts";
}
...
fs::path autosave_dir() {
  return layouts_dir() / ".jotplugger_autosave";
}
...
ImGuiID dockspace_id_for_tab(int tab_runtime_id) {
  return ImHashStr(util::string_format("jotplugger_dockspace_%d", tab_runtime_id).c_str());
}
