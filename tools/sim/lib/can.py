...
def can_function(pm, speed, angle, idx, cruise_button, is_engaged):
  ...
  checksum = crc8_pedal(packer.make_can_msg("INTERCEPTOR_GAS_SENSOR", 0, {"COUNTER_PEDAL": idx & 0xF}, -1)[2][:-1])
  values["CHECKSUM_PEDAL"] = checksum
  msg.append(packer.make_can_msg("INTERCEPTOR_GAS_SENSOR", 0, values, -1))
...
def sendcan_function(sendcan):
  ...
  if cp.vl[0x200]['INTERCEPTOR_GAS_COMMAND'] > 0:
    gas = ( cp.vl[0x200]['INTERCEPTOR_GAS_COMMAND'] + 83.3 ) / (0.253984064 * 2**16)
  else:
    gas = 0.0
