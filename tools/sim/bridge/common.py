  ...
  def _run(self, q: Queue):
    ...
    while self._keep_alive:
      ...
      if self.simulator_state.is_engaged:
        throttle_op = float(np.clip(self.simulated_car.sm['carControl'].actuators.accel / 1.6, 0.0, 1.0))
        brake_op = float(np.clip(-self.simulated_car.sm['carControl'].actuators.accel / 4.0, 0.0, 1.0))
