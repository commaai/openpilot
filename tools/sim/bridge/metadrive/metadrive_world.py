...
class MetaDriveWorld(World):
  ...
  def read_sensors(self, state: SimulatorState):
    while self.vehicle_state_recv.poll(0):
      ...
      # Update SimulatorState with MetaDrive vehicle state
      ...
      # Populate IMUState (faking accelerometer, gyroscope, and bearing)
      state.imu.accelerometer = md_vehicle.velocity  # Example: Adjust based on velocity changes
      state.imu.gyroscope = vec3(0, 0, md_vehicle.steering_angle)  # Example: Simplify for now
      ...
      # Check if the vehicle has moved after being engaged
      ...
      # Periodically check if the vehicle is moving
