...
if [ -n "$DEVELOP" ]; then
    ...
    docker run  --shm-size 1G --rm --net=host -e PASSIVE=0 -e NOBOARD=1 -e CUDA_VISIBLE_DEVICES=  --volume="$HOME/openpilot:/tmp/openpilot-sim" --volume="$HOME/.Xauthority:/root/.Xauthority:rw" --gpus all -e DISPLAY=$DISPLAY -it commaai/openpilot-sim:latest /bin/bash -c "screen -d -m python tmp/openpilot-sim/selfdrive/manager.py && screen python tmp/openpilot-sim/tools/sim/bridge.py"
    
else 
    echo "Run openpilot-sim with manager and bridge"
    docker run  --shm-size 1G --rm --net=host -e PASSIVE=0 -e NOBOARD=1 -e CUDA_VISIBLE_DEVICES=  --volume="$HOME/.Xauthority:/root/.Xauthority:rw" --gpus all -e DISPLAY=$DISPLAY -it commaai/openpilot-sim:latest /bin/bash -c "screen -d -m python tmp/openpilot/selfdrive/manager.py && screen python tmp/openpilot/tools/sim/bridge.py"
fi
