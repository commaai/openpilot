...
def cycle_alerts(duration=200, is_metric=False):
  ...
  sm = messaging.SubMaster(['deviceState', 'pandaStates', 'narrowRoadCameraState', 'modelV2', 'cameraCalibration',
                            'driverMonitoringState', 'longitudinalPlan', 'devicePose',
  ...
  while True:
    for alert, et in alerts:
      ...
      sm['cameraCalibration'].rpyCalib = [-1 * random.random() for _ in range(random.randint(0, 3))]
