...
FRAMERATE = 20
CAM_WIDTH, CAM_HEIGHT = 1928, 1208
QCAM_WIDTH, QCAM_HEIGHT = 526, 330
...
def parse_args():
  ...
  parser.add_argument("--prefix", default="clip_direct", help="Openpilot prefix")
  ...
  for global_idx in range(start_frame, end_frame):
    ...
    if seg_idx != current_seg:
      ...
      if use_qcam:
        ...
        seg_frames = np.frombuffer(result.stdout, dtype=np.uint8).reshape(-1, QCAM_WIDTH * QCAM_HEIGHT * 3 // 2)
      else:
...
class FrameQueue:
  def __init__(self, camera_paths, start_time, end_time, fps=20, prefetch_count=60, use_qcam=False):
    ...
    self._thread = threading.Thread(target=self._worker, args=(camera_paths, start_time, end_time, fps, use_qcam), daemon=True)
    self._thread.start()

  def _worker(self, camera_paths, start_time, end_time, fps, use_qcam):
    try:
      for idx, data in iter_segment_frames(camera_paths, start_time, end_time, fps, use_qcam):
...
def clip(route: Route, output: str, start: int, end: int, headless: bool = True, prefix: str = "clip", big: bool = False,
  ...
  with OpenpilotPrefix(prefix, shared_download_cache=True):
    ...
    cam_w, cam_h = (QCAM_WIDTH, QCAM_HEIGHT) if use_qcam else (CAM_WIDTH, CAM_HEIGHT)
    vipc = VisionIpcServer("camerad")
    vipc.create_buffers(VisionStreamType.VISION_STREAM_ROAD, 4, cam_w, cam_h)
    vipc.start_listener()
...
def main():
  ...
  clip(Route(args.route, data_dir=args.data_dir), args.output, args.start, args.end, not args.windowed,
       args.prefix, args.big, args.title, not args.no_metadata, not args.no_time_overlay, args.qcam)
