...
int main() {
  ...
  printf("got gl texture %d\n", whichID);
  SDL_GL_UnbindTexture(texture);

  // link the texture to CUDA
  CUgraphicsResource res;
  CHECK(cuGraphicsGLRegisterImage(&res, whichID, GL_TEXTURE_2D, CU_GRAPHICS_REGISTER_FLAGS_WRITE_DISCARD));
  CHECK(cuGraphicsMapResources(1, &res, NULL));
  CHECK(cuGraphicsSubResourceGetMappedArray(&texture_cuda, res, 0, 0));
  CUDA_ARRAY_DESCRIPTOR pArrayDescriptor;
  CHECK(cuArrayGetDescriptor(&pArrayDescriptor, texture_cuda));
  printf("dst array is %p = %dx%d %d\n", texture_cuda,
