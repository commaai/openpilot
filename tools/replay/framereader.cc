...
bool FrameReader::loadFromFile(CameraType type, const std::string &file, bool no_hw_decoder, std::atomic<bool> *abort) {
  ...
  if (video_stream_index < 0) {
    rError("No video stream found");
    return false;
  }

  decoder_ = decoder_manager.acquire(type, input_ctx->streams[video_stream_index]->codecpar, !no_hw_decoder);
  if (!decoder_) {
    return false;
  }
...
bool VideoDecoder::decode(FrameReader *reader, int idx, VisionBuf *buf) {
  ...
  bool result = false;
  AVPacket pkt;
  int frames_to_skip = idx - from_idx;
  while (frames_to_skip >= 0) {
    if (av_read_frame(reader->input_ctx, &pkt) < 0) break;
