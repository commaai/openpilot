...
const int NUM_TEST_SEGMENTS = std::getenv("NUM_TEST_SEGMENTS") ? atoi(std::getenv("NUM_TEST_SEGMENTS")) : 1; // number of segments to test replay with
...
TEST_CASE("Local route") {
  ...
  for (int i = 0; i < NUM_TEST_SEGMENTS; ++i) {
