...
class TestPlotJuggler(unittest.TestCase):
  ...
  def test_run(self):
    ...
    p = subprocess.Popen(f'QT_QPA_PLATFORM=offscreen {os.path.join(BASEDIR, "tools/plotjuggler/juggle.py")} "{test_url}"', stderr=subprocess.PIPE, stdout=subprocess.PIPE ,shell=True, start_new_session=True)
