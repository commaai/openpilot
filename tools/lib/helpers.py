...
class RE:
  ...
  SEGMENT_NUMBER = r'(?P<segment_num>[0-9]+)'

  FILE_NAME = r'(?P<file_name>(?:[fde]camera.hevc|[qr]log.bz2))'

  ROUTE_NAME = r'(?P<route_name>{}[|_/]{})'.format(DONGLE_ID, LOG_ID)

  SEGMENT_NAME = r'(?P<segment_name>{}(?:--|/){})'.format(ROUTE_NAME, SEGMENT_NUMBER)
  ...
  EXPLORER_FILE = r'^{}--{}$'.format(SEGMENT_NAME, FILE_NAME)
