...
class WifiManager:
  ...
  def _handle_state_change(self, new_state: int, prev_state: int, change_reason: int):
    ...
    #   ACTIVATED: reads _wifi_state, does slow DBus call, writes back — if
    #   _set_connecting runs in between, the handler overwrites it with stale state.
    # The deterministic fixes below (skip DBus lookup when ssid set, DEACTIVATING no-op,
    # CONNECTION_REMOVED guard) shrink the race window to near-zero for PREPARE/CONFIG.
    # ACTIVATED still has a small window around its DBus call. If still visible, add an
    # epoch counter or narrow lock around _wifi_state reads/writes (not around DBus calls).
    ...
    if new_state == NMDeviceState.DISCONNECTED:
      ...
      if self._wifi_state.ssid is not None:
        ...
        # Skip DBus lookup to avoid overwriting with stale conn_path from a prior connection.
