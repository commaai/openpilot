...
class GuiScrollPanel2:
  ...
  def _update_state(self, bounds_size: float, content_size: float) -> None:
    ...
    if self._state == ScrollState.AUTO_SCROLL:
      ...
      out_of_bounds = self._get_offset() > 0 or self._get_offset() < (bounds_size - content_size)
      if out_of_bounds and self._handle_out_of_bounds:
        if self._get_offset() < (bounds_size - content_size):  # too far right
        ...
        dist = target - self._get_offset()
        self.set_offset(self._get_offset() + dist * factor)  # ease toward the edge
      ...
      self.set_offset(self._get_offset() + self._velocity * dt)  # Adjust the offset based on velocity
  ...
  def _handle_mouse_event(self, mouse_event: MouseEvent, bounds: rl.Rectangle, bounds_size: float,
    ...
    if not self.enabled:
      ...
      if mouse_event.left_released:
        ...
        # Use internal _offset directly to preserve precision (don't round via _get_offset())
        # TODO: make _get_offset return float
