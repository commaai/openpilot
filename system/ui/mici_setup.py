...
class NetworkSetupPageBase(Scroller):
  ...
  def show_event(self):
    ...
    def scroll_if_has_internet():
      has_internet = self._has_internet
      if has_internet:
        self._scroll_to_end()

    self.set_shown_callback(scroll_if_has_internet)
  ...
  def _nav_stack_tick(self):
    ...
    if has_internet:
      if not self._prev_has_internet:
        self._pending_has_internet_scroll = rl.get_time()
    elif wifi_connected:
      if not self._prev_wifi_connected:
        self._pending_has_internet_scroll = rl.get_time()
    ...
    if self._pending_has_internet_scroll is not None:
      ...
      gui_app.pop_widgets_to(self, self._scroll_to_end)
