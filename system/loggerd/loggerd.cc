...
void handle_user_bookmark(LoggerdState *s) {
...
void loggerd_thread() {
  ...
  typedef struct ServiceState {
    std::string name;
    int counter, freq;
    bool encoder, user_bookmark, record_audio;
  } ServiceState;
  ...
  for (const auto& [_, it] : services) {
    ...
    if (it.should_log || (encoder && !livestream_encoder) || record_audio) {
      ...
      service_state[sock] = {
        ...
        .freq = it.decimation,
        .encoder = encoder,
        .user_bookmark = it.name == "userBookmark",
  ...
  while (!do_exit) {
    ...
    for (auto sock : poller->poll(1000)) {
      ...
      if (service.user_bookmark) {
        handle_user_bookmark(&s);
      }
