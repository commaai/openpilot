...
const int env_debug_encoder_burn = (getenv("DEBUG_ENCODER_BURN") != NULL) ? atoi(getenv("DEBUG_ENCODER_BURN")) : 0;
...
void V4LEncoderBurn::dequeue_handler(V4LEncoderBurn *e) {
  std::string dequeue_thread_name = "burnq-"+std::string(e->encoder_info.publish_name);
  ...
  while (!exit) {
    int rc = poll(&pfd, 1, 100); // Reduced timeout for more aggressive polling
    if (rc < 0) {
      if (errno != EINTR) {
      ...
      // More frequent timeout for burn test
      continue;
    }

    if (env_debug_encoder_burn >= 2) {
      printf("%20s BURN poll %x at %.2f ms\n", e->encoder_info.publish_name, pfd.revents, millis_since_boot());
    }
    ...
    if (pfd.revents & POLLIN) {
      ...
      if (env_debug_encoder_burn) {
        printf("%20s BURN got(%d) %6d bytes flags %8x idx %3d/%4d id %8d ts %ld lat %.2f ms (%lu frames free)\n",
      ...
      // requeue the buffer immediately for stress testing
...
V4LEncoderBurn::V4LEncoderBurn(const EncoderInfo &encoder_info, int in_width, int in_height)
  ...
  printf("Line %d\n", __LINE__);
  LOGD("opened BURN encoder device %s %s = %d", cap.driver, cap.card, fd);
  ...
  v4l2_streamparm streamparm = {
    ...
    .parm = {
      .output = {
        .timeperframe = {
          .numerator = 1,
          .denominator = (unsigned int)encoder_info.fps // Use provided fps for stress testing
        }
      }
    }
  };
  ...
  LOGD("BURN in buffer size %d, out buffer size %d",
    fmt_in.fmt.pix_mp.plane_fmt[0].sizeimage,
    fmt_out.fmt.pix_mp.plane_fmt[0].sizeimage);

  // Aggressive encoder settings for maximum power draw
  {
    struct v4l2_control ctrls[] = {
      { .id = V4L2_CID_MPEG_VIDEO_HEADER_MODE, .value = V4L2_MPEG_VIDEO_HEADER_MODE_SEPARATE},
      { .id = V4L2_CID_MPEG_VIDEO_BITRATE, .value = encoder_info.bitrate * 4}, // 4x bitrate for stress
  ...
  printf("Line %d\n", __LINE__);
    ...
    struct v4l2_control ctrls[] = {
      ...
      { .id = V4L2_CID_MPEG_VIDEO_H264_LEVEL, .value = V4L2_MPEG_VIDEO_H264_LEVEL_5_2}, // Level 5.2 for 4K
      { .id = V4L2_CID_MPEG_VIDC_VIDEO_NUM_P_FRAMES, .value = 0}, // All I-frames for maximum processing
      { .id = V4L2_CID_MPEG_VIDC_VIDEO_NUM_B_FRAMES, .value = 0},
      ...
      { .id = V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_MODE, .value = 1}, // Enable loop filter for more processing
      { .id = V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_ALPHA, .value = 6},
      { .id = V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_BETA, .value = 6},
  ...
  // allocate more buffers for stress testing
  request_buffers(fd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, BURN_BUF_OUT_COUNT);
  request_buffers(fd, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE, BURN_BUF_IN_COUNT);
  ...
  for (unsigned int i = 0; i < BURN_BUF_OUT_COUNT; i++) {
  ...
  for (unsigned int i = 0; i < BURN_BUF_IN_COUNT; i++) {
    free_buf_in.push(i);
  }
}

void V4LEncoderBurn::encoder_open() {
  dequeue_handler_thread = std::thread(V4LEncoderBurn::dequeue_handler, this);
  this->is_open = true;
  this->counter = 0;
}

int V4LEncoderBurn::encode_frame(VisionBuf* buf, VisionIpcBufExtra *extra) {
  struct timeval timestamp {
...
void V4LEncoderBurn::encoder_close() {
  if (this->is_open) {
    // pop all the frames before closing, then put the buffers back
    for (int i = 0; i < BURN_BUF_IN_COUNT; i++) free_buf_in.pop();
    for (int i = 0; i < BURN_BUF_IN_COUNT; i++) free_buf_in.push(i);
...
V4LEncoderBurn::~V4LEncoderBurn() {
  ...
  for (int i = 0; i < BURN_BUF_OUT_COUNT; i++) {
    if (buf_out[i].free() != 0) {
