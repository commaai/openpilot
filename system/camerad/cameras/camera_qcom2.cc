...
public:
  ...
  CameraState(SpectraMaster *master, const CameraConfig &config) : camera(master, config, config.stream_type == VISION_STREAM_ROAD ? ISP_BPS_PROCESSED : ISP_IFE_PROCESSED) {};
  ~CameraState();
