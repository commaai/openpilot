...
void SpectraCamera::config_ipe(int io_mem_handle, int request_id, int idx) {
  ...
  */
  ...
  pkt->header.op_code = 0x10000000;  // (CSLDeviceTypeICP(0x10) << 24) | CSLPacketOpcodesIPEUpdate (0)
  pkt->header.request_id = request_id;
  pkt->header.size = size;
  ...
  // *** cmd buf ***
  {
    ...
    // looks like we might not have these structs :(
    // see camxcmdbuffer.h
    // *** first command ***
    // TODO: support MMU
    ...
    tmp.header = 0x1; // CSLICPGenericBlobCmdBufferClk
    ...
    buf_desc[1].meta_data = CAM_ISP_PACKET_META_LEFT; //CAM_ISP_PACKET_META_GENERIC_BLOB_COMMON;
  ...
  // *** io config ***
  {
    ...
    io_cfg[1].planes[0] = (struct cam_plane_cfg){
      ...
      .slice_height = ALIGNED_SIZE(sensor->frame_height + sensor->extra_height, 0x20),
    };
  ...
  */
  ...
  // *** cmd buf ***
  {
    ...
    */
    ...
    if (init) {
      buf_desc[0].length = sizeof(isp_ife_init)-1;
      isp_prog = isp_ife_init;
    } else {
...
void SpectraCamera::camera_close() {
  ...
  if (enabled) {
    ...
    LOGD("stop icp: %d", ret);
    ret = device_control(m->isp_fd, CAM_STOP_DEV, session_handle, isp_dev_handle);
    LOGD("stop isp: %d", ret);
