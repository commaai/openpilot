...
void SpectraCamera::config_bps(int idx, int request_id) {
  ...
  */
  ...
  int size = sizeof(struct cam_packet) + sizeof(struct cam_cmd_buf_desc)*2 + sizeof(struct cam_buf_io_cfg)*3;
  size += sizeof(struct cam_patch_desc)*14;
  ...
  struct cam_cmd_buf_desc *buf_desc = (struct cam_cmd_buf_desc *)&pkt->payload;
  {
    ...
    tmp.clk.frame_cycles = 4085760; // comes from the striping lib
    tmp.clk.rt_flag = 0x0;
  ...
  struct cam_buf_io_cfg *io_cfg = (struct cam_buf_io_cfg *)((char*)&pkt->payload + pkt->io_configs_offset);
  {
    ...
    io_cfg[2].mem_handle[0] = bps_full_scratch.handle;
    io_cfg[2].mem_handle[1] = bps_full_scratch.handle;
  ...
  // *** patches ***
  {
    ...
    add_patch(pkt.get(), bps_cmd.handle, buf_desc[0].offset + offsetof(bps_tmp, frames[1].ptr[0]), bps_full_scratch.handle, 0);
    add_patch(pkt.get(), bps_cmd.handle, buf_desc[0].offset + offsetof(bps_tmp, frames[1].ptr[1]), bps_full_scratch.handle, io_cfg[2].offsets[1]);
...
void SpectraCamera::configICP() {
  ...
  */
  ...
  uint32_t full_stride, full_y_h, full_uv_h, full_yuv_size;
  std::tie(full_stride, full_y_h, full_uv_h, full_yuv_size) = get_nv12_info(sensor->frame_width, sensor->frame_height);
  bps_full_scratch.init(m, full_yuv_size, 0x1000, true, m->icp_device_iommu);
