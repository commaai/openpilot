...
from openpilot.system.webrtc.session_manager import _create_session_async
...
class TestSessionManager:
  """Tests the in-process session manager path (no HTTP server)."""
  ...
  async def test_create_session(self, mocker):
    from openpilot.system.webrtc import session_manager

    async def connect(offer):
      answer = await _create_session_async(offer.sdp, offer.video, self.in_services, self.out_services)
      return aiortc.RTCSessionDescription(**answer)
    ...
    # cleanup sessions created by session_manager
