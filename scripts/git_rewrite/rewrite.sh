...
cp -r openpilot openpilot_backup
cd openpilot

git checkout master

# delete all refs except master
git for-each-ref --format='delete %(refname)' | grep -v '\/master$' | git update-ref --stdin
