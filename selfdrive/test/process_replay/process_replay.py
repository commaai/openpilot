...
from typing import Dict, List, Optional, Callable, Union, Any, Iterable
...
class ProcessContainer:
  def __init__(self, cfg: ProcessConfig):
    ...
    self.sockets: Optional[Dict[str, messaging.SubSocket]] = None
...
def _replay_multi_process(
  ...
  try:
    ...
    while len(external_pub_queue) != 0 or (len(internal_pub_queue) != 0 and not all(c.has_empty_queue for c in containers)):
      if len(internal_pub_queue) == 0 or (len(external_pub_queue) != 0 and external_pub_queue[0].logMonoTime < internal_pub_queue[0].logMonoTime):
