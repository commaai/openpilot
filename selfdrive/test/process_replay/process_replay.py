...
def _replay_multi_process(
  ...
  try:
    ...
    internal_pub_queue: List[Tuple[int, capnp._DynamicStructReader]] = []

    pbar = tqdm(total=len(external_pub_queue), disable=disable_progress)
    while len(external_pub_queue) != 0 or (len(internal_pub_queue) != 0 and not all(c.has_empty_queue for c in containers)):
      if len(internal_pub_queue) == 0 or (len(external_pub_queue) != 0 and external_pub_queue[0].logMonoTime < internal_pub_queue[0][1].logMonoTime):
        msg = external_pub_queue.pop(0)
        pbar.update(1)
      else:
        msg = heapq.heappop(internal_pub_queue)[1]
      ...
      for container in target_containers:
        ...
        for m in output_msgs:
          if m.which() in all_pubs:
            heapq.heappush(internal_pub_queue, (m.logMonoTime, m))
