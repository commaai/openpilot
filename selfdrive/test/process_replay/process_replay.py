...
class MultiProcessReplaySession:
  def __init__(
    ...
    self.captured_output_store = captured_output_store
    self.disable_progress = disable_progress
    self.return_all_logs = return_all_logs
  ...
  def _start(self, first_input_segment):
    ...
    # start containers with the *same* segment
    self.containers = []
    for cfg in self.cfgs:
      c = ProcessContainer(cfg)
      c.start(self.params_config, self.env_config,
              first_input_segment, self.frs, self.fingerprint,
              self.captured_output_store is not None)
      self.containers.append(c)

    self._started = True

  def _get_configs(self, msgs):
    # Use the first segment's messages to generate configs
    if self.fingerprint is not None:
      params_config = generate_params_config(lr=msgs, fingerprint=self.fingerprint, custom_params=self.custom_params)
      env_config = generate_environ_config(fingerprint=self.fingerprint)
    else:
      CP = next((m.carParams for m in msgs if m.which() == "carParams"), None)
      params_config = generate_params_config(lr=msgs, CP=CP, custom_params=self.custom_params)
      env_config = generate_environ_config(CP=CP)
    return params_config, env_config

  def _feed(self, segment):
    # Feed a batch of messages (from a segment) to the running processes
    if not self._started:
      self._start(segment)
    all_pubs = {pub for container in self.containers for pub in container.pubs}
    all_subs = {sub for container in self.containers for sub in container.subs}
    lr_pubs = all_pubs - all_subs
    pubs_to_containers = {pub: [container for container in self.containers if pub in container.pubs] for pub in all_pubs}

    pub_msgs = [msg for msg in segment if msg.which() in lr_pubs]
    external_pub_queue = pub_msgs.copy()
    internal_pub_queue = []
    internal_pub_index_heap = []
    log_msgs = []
    pbar = tqdm(total=len(external_pub_queue), disable=self.disable_progress)
    while len(external_pub_queue) != 0 or (len(internal_pub_index_heap) != 0 and not all(c.has_empty_queue for c in self.containers)):
      ...
      for container in target_containers:
        output_msgs = container.run_step(msg, self.frs)
    ...
    if self.return_all_logs:
      # combine the logs from the migrated segment and the logs from the target process and sort them by logMonoTime
      keys = {m.which() for m in log_msgs}
      modified_logs = [m for m in segment if m.which() not in keys]
      modified_logs.extend(log_msgs)
      modified_logs.sort(key=lambda m: int(m.logMonoTime))
      yield modified_logs
    else:  # returns the logs from the target process only
      yield log_msgs

  def _stop(self):
    for container in self.containers:
      container.stop()
      if self.captured_output_store is not None:
