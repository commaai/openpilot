...
from selfdrive.locationd.calibrationd import calibrationd_thread
...
if __name__ == "__main__":
  ...
  # Statistical
  sm, pm, can_sock = get_inputs(msgs, 'calibrationd')
  with pprofile.StatisticalProfile()(period=0.00001) as pr:
    try:
      calibrationd_thread(sm, pm)
    except ReplayDone:
      pass
  pr.dump_stats('cachegrind.out.calibrationd_statistical')
  ...
  with cProfile.Profile() as pr:
    try:
      ...
      pass
  pyprof2calltree.convert(pr.getstats(), 'cachegrind.out.calibrationd_deterministic')
