...
commit_root() {
    ...
    target="/overlay"
    sudo mount -t overlay overlay -o lowerdir=/,upperdir=/upper,workdir=/work "$target" # mount the overlayfs
    mounts+=("$target")

    echo "$org_mounts" | cut -d" " -f2 | while read line # bind-mount any mounts under the old rootfs into the new one (overlayfs isn't recursive like e.g. `mount --rbind`)
    do
        if [ "$line" != "/" ] # except the rootfs base, to avoid infinite mountpoint loops
        then
            target="/overlay$line"
            sudo mount --bind "$line" "$target"
            mounts+=("$target")
        fi
    done
...
if false #[ -f "$CACHE_ROOTFS_TARBALL_PATH" ]
then
