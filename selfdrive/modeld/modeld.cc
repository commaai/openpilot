...
void run_model(ModelState &model, VisionIpcClient &vipc_client) {
  ...
  while (!do_exit) {
    ...
    if (run_model_this_iter) {
      ...
      auto model_data = model_eval_frame(&model, buf->buf_cl, buf->width, buf->height,
      ...
      model_publish(pm, extra.frame_id, frame_id, frame_drop_ratio, *model_data, extra.timestamp_eof, model_execution_time,
                    kj::ArrayPtr<const float>(model.output.data(), model.output.size()));
      posenet_publish(pm, extra.frame_id, vipc_dropped_frames, *model_data, extra.timestamp_eof);
