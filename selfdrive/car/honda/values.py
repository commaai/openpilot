...
class HondaCarInfo(CarInfo):
  ...
  def init_make(self, CP: car.CarParams):
    if CP.carFingerprint in HONDA_BOSCH:
      harness = CarHarness.bosch_b if CP.carFingerprint in HONDA_BOSCH_RADARLESS else CarHarness.bosch_a
    else:
      harness = CarHarness.nidec

    device = Device.three
    if CP.carFingerprint in (CAR.HRV_3G,):
      device = Device.three_angled_mount

    self.car_parts = CarParts([device, harness])
...
CAR_INFO: Dict[str, Optional[Union[HondaCarInfo, List[HondaCarInfo]]]] = {
  ...
  CAR.HRV: HondaCarInfo("Honda HR-V 2019-22", car_parts=CarParts([Device.three_angled_mount, CarHarness.ford_q3]), min_steer_speed=12. * CV.MPH_TO_MS),
