...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd,
    ...
    lkas_active = actuators.lat_active and not CS.steer_not_allowed
    ...
    if not CS.CP.openpilotLongitudinalControl:
      ...
      if (frame % 2) == 0:
        ...
        if CS.CP.carFingerprint in HONDA_BOSCH:
          ...
          can_sends.extend(hondacan.create_acc_commands(self.packer, enabled, actuators.long_active, accel, self.gas, idx, stopping, CS.CP.carFingerprint))
        else:
