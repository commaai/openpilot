    ...
    for e in [True, False]:
      ...
      for f in [True, False]:
        ...
        if self.CP.minSteerSpeed < 6.:
          ...
          # same, show alert
          for _ in ((self.CP.minSteerSpeed-1),
                    (self.CP.minSteerSpeed)):
            self._speed(_)
            self._update_events()
            assert BelowSteerSpeed in self.CS.events.names

          # just over min steer
          # fast first time, below threshold
          # slower again
          for _ in ((self.CP.minSteerSpeed + 0.001),
          ...
          # faster still, but over the pre_alert threshold
          # now slow just under the pre_alert threshold
          # even slower still
          for _ in [(self.CP.minSteerSpeed + 3.6),
                    (self.CP.minSteerSpeed + 3.4),
                    (self.CP.minSteerSpeed + 1.6)]:
            self._speed(_)
            self._update_events()
            assert BelowSteerSpeed not in self.CS.events.names
          ...
          for _ in [(self.CP.minSteerSpeed + 1.5),
                    (self.CP.minSteerSpeed + 1.4)]:
            self._speed(_)
            self._update_events()
            if self.car_events.min_steer_alert_speed != self.CP.minSteerSpeed:
              assert BelowSteerSpeed in self.CS.events.names
            else:
              assert BelowSteerSpeed not in self.CS.events.names
