...
def match_fw_to_car(fw_versions: list[capnp.lib.capnp._DynamicStructBuilder], vin: str,
  ...
  for exact_match, match_func in exact_matches:
    ...
    for brand in VERSIONS.keys():
      ...
      if exact_match and not len(matches) and config.match_fw_to_car_custom is not None:
        matches |= config.match_fw_to_car_custom(fw_versions_dict, vin, VERSIONS[brand])
