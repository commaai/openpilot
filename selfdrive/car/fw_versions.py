...
def match_fw_to_car(fw_versions, allow_exact=True, allow_fuzzy=True, log=True):
  ...
  brand_to_fw_dict = {brand: build_fw_dict(fw_versions, filter_brand=brand) for brand in VERSIONS.keys()}

  for exact_match, match_func in exact_matches:
    # For each brand, attempt to fingerprint using all FW returned from its queries
    matches = set()
    for brand in VERSIONS.keys():
      matches |= match_func(brand_to_fw_dict[brand], match_brand=brand, log=log)
      ...
      if not exact_match and not len(matches) and config.match_fw_to_car_fuzzy is not None:
        matches |= config.match_fw_to_car_fuzzy(brand_to_fw_dict[brand])
