...
def get_brand_addrs():
  ...
  for brand, config, r in REQUESTS:
    for brand_versions in VERSIONS[brand].values():
      ...
      for ecu_type, addr, sub_addr in (list(brand_versions.keys()) + config.extra_ecus):
...
def get_brand_ecu_matches(ecu_rx_addrs: Set[EcuAddrBusType]) -> Dict[str, Set[EcuAddrBusType]]:
  """Returns dictionary of brands and matches with ECUs in their FW versions"""

  brand_addrs = get_brand_addrs()
  brand_matches: Dict[str, Set[EcuAddrBusType]] = {brand: set() for brand, _, _ in REQUESTS}
