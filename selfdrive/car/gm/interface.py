...
class CarInterface(CarInterfaceBase):
  ...
  def torque_from_lateral_accel_gm(lateral_accel_value, torque_params, lateral_accel_error, lateral_accel_deadzone, speed, friction_compensation):
    ...
    curve_steepness = 1 / (speed ** 1 + 2) + 1  # 1.75 - curve_drop_off  # higher steepness with less drop off
    if lateral_accel_value != 0:
      torque = (1.5 * lateral_accel_value) / (1 + abs(lateral_accel_value) ** curve_drop_off) / (torque_params.latAccelFactor * curve_steepness)
