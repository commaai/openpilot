...
def create_throttle(packer, throttle_msg, throttle_cmd, bus):
  ...
  values["COUNTER"] = (values["COUNTER"] + 2) % 0x10 # +1 didn't work consistently
  
  if throttle_cmd:
    values["Throttle_Pedal"] = 5

  return packer.make_can_msg("Throttle", bus, values)
