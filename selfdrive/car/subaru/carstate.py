...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    throttle_msg = cp.vl["Throttle"] if not SubaruFlags.HYBRID.check(self.CP.flags) else cp_body.vl["Throttle_Hybrid"]
    ...
    if SubaruFlags.PREGLOBAL.check(self.CP.flags):
      ...
      cp_brakes = cp_body if SubaruFlags.GLOBAL_GEN2.check(self.CP.flags) else cp
      ret.brakePressed = cp_brakes.vl["Brake_Status"]["Brake"] == 1

    cp_wheels = cp_body if SubaruFlags.GLOBAL_GEN2.check(self.CP.flags) else cp
    ...
    cp_transmission = cp_body if SubaruFlags.HYBRID.check(self.CP.flags) else cp
    can_gear = int(cp_transmission.vl["Transmission"]["Gear"])
    ret.gearShifter = self.parse_gear_shifter(self.shifter_values.get(can_gear, None))

    ret.steeringAngleDeg = cp.vl["Steering_Torque"]["Steering_Angle"]

    if not (SubaruFlags.PREGLOBAL.check(self.CP.flags)):
    ...
    steer_threshold = 75 if SubaruFlags.PREGLOBAL.check(self.CP.flags) else 80
    ret.steeringPressed = abs(ret.steeringTorque) > steer_threshold

    cp_cruise = cp_body if SubaruFlags.GLOBAL_GEN2.check(self.CP.flags) else cp
    if SubaruFlags.HYBRID.check(self.CP.flags):
    ...
    if (SubaruFlags.PREGLOBAL.check(self.CP.flags) and cp.vl["Dash_State2"]["UNITS"] == 1) or \
       (not SubaruFlags.PREGLOBAL.check(self.CP.flags) and cp.vl["Dashlights"]["UNITS"] == 1):
    ...
    if SubaruFlags.PREGLOBAL.check(self.CP.flags):
      ...
      cp_es_brake = cp_body if SubaruFlags.GLOBAL_GEN2.check(self.CP.flags) else cp_cam
      self.es_brake_msg = copy.copy(cp_es_brake.vl["ES_Brake"])
      cp_es_status = cp_body if SubaruFlags.GLOBAL_GEN2.check(self.CP.flags) else cp_cam
  ...
  def get_common_global_body_messages(CP):
    ...
    if not SubaruFlags.HYBRID.check(CP.flags):
  ...
  def get_can_parser(CP):
    ...
    if not SubaruFlags.PREGLOBAL.check(CP.flags):
      if not SubaruFlags.GLOBAL_GEN2.check(CP.flags):
  ...
  def get_cam_can_parser(CP):
    if CP.flags & SubaruFlags.PREGLOBAL:
      ...
      if SubaruFlags.SEND_INFOTAINMENT.check(CP.flags):
  ...
  def get_body_can_parser(CP):
    ...
    if SubaruFlags.HYBRID.check(CP.flags):
      messages += [
        ("Throttle_Hybrid", 40),
        ("Transmission", 100)
      ]
