  ...
  def update(self, CC, CS, now_nanos):
    ...
    if (self.frame % self.p.STEER_STEP) == 0:
      ...
      if SubaruFlags.PREGLOBAL.is_set(self.CP.flags):
        ...
        if SubaruFlags.STEER_RATE_LIMITED.is_set(self.CP.flags):
      ...
      if self.CP.openpilotLongitudinalControl:
        ...
        if pcm_cancel_cmd:
          if not SubaruFlags.HYBRID.is_set(self.CP.flags):
            bus = CanBus.alt if SubaruFlags.GLOBAL_GEN2.is_set(self.CP.flags) else CanBus.main
            can_sends.append(subarucan.create_es_distance(self.packer, CS.es_distance_msg["COUNTER"] + 1, CS.es_distance_msg, bus, pcm_cancel_cmd))

      if SubaruFlags.DISABLE_EYESIGHT.is_set(self.CP.flags):
