...
from openpilot.selfdrive.car import make_tester_present_msg, logging
...
def get_ecu_addrs(logcan: messaging.SubSocket, sendcan: messaging.PubSocket, queries: set[EcuAddrBusType],
  ...
  try:
    ...
    while time.monotonic() - start_time < timeout:
      ...
      for packet in can_packets:
        for msg in packet.can:
          if not len(msg.dat):
            logging.warning("ECU addr scan: skipping empty remote frame")
            continue
    ...
    logging.exception("ECU addr scan exception")
