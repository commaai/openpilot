    ...
    while True:
      ...
      for tx_addr, msg in msgs.items():
        try:
          ...
          logging.exception(f"Error processing UDS response: {tx_addr}")
        ...
        if len(dat) == 0:
          logging.error(f"iso-tp query empty response: {tx_addr}")
          request_done[tx_addr] = True
          continue
        ...
        if response_valid:
          ...
          if error_code == 0x78:
            ...
            logging.error(f"iso-tp query response pending: {tx_addr}")
          else:
            request_done[tx_addr] = True
            logging.error(f"iso-tp query bad response: {tx_addr} - 0x{dat.hex()}")
        ...
        if cur_time - response_timeouts[tx_addr] > 0:
          if not request_done[tx_addr]:
            if request_counter[tx_addr] > 0:
              logging.error(f"iso-tp query timeout after receiving partial response: {tx_addr}")
            elif tx_addr in addrs_responded:
              logging.error(f"iso-tp query timeout while receiving response: {tx_addr}")
            # TODO: handle functional addresses
            # else:
            #   logging.error(f"iso-tp query timeout with no response: {tx_addr}")
          request_done[tx_addr] = True
      ...
      if cur_time - start_time > total_timeout:
        logging.error("iso-tp query timeout while receiving data")
        break
