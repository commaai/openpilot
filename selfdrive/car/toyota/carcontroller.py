...
class CarController():
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    if CS.CP.enableGasInterceptor and active:
      ...
      if CS.CP.carModel in (CAR.RAV4, CAR.RAV4H, CAR.HIGHLANDER, CAR.HIGHLANDERH):
        PEDAL_SCALE = interp(CS.out.vEgo, [0.0, MIN_ACC_SPEED, MIN_ACC_SPEED + PEDAL_TRANSITION], [0.15, 0.3, 0.0])
      elif CS.CP.carModel in (CAR.COROLLA,):
    ...
    if CS.out.standstill and not self.last_standstill and CS.CP.carModel not in NO_STOP_TIMER_CAR:
    ...
    if frame % 2 == 0 and CS.CP.carModel in TSS2_CAR:
    ...
    if (frame % 3 == 0 and CS.CP.openpilotLongitudinalControl) or pcm_cancel_cmd:
      ...
      if pcm_cancel_cmd and CS.CP.carModel in (CAR.LEXUS_IS, CAR.LEXUS_RC):
    ...
    for (addr, cars, bus, fr_step, vl) in STATIC_DSU_MSGS:
      if frame % fr_step == 0 and CS.CP.enableDsu and CS.CP.carModel in cars:
