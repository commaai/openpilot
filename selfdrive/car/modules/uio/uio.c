...
static void bb_ui_draw_UI(UIState *s, BBUIVals *v) {
 ...
 static void bb_ui_play_sound(UIState *s, BBUIVals *v, int sound) {
   ...
   if ((bts > 0) || (bts == -1)) {
    asprintf(&snd_command, "python /data/openpilot/selfdrive/car/common/snd/playsound.py %d &", sound);
    system(snd_command);
   }
 }
 ...
 static void  bb_ui_poll_update(UIState *s, BBUIVals *v) {
    ...
    while (true) {
        ...
        bb_polls[1].socket = v->uiCustomAlert_sock;
        bb_polls[1].events = ZMQ_POLLIN;
        bb_polls[2].socket = v->uiSetCar_sock;
        bb_polls[2].events = ZMQ_POLLIN;
        bb_polls[3].socket = v->uiPlaySound_sock;
        ...
        bb_polls[6].socket = s->live100_sock;
