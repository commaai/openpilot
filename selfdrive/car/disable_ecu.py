...
def disable_ecu(logcan, sendcan, bus=0, addr=0x7d0, sub_addr=None, com_cont_req=b'\x28\x83\x01', timeout=0.1, retry=10, debug=False):
  ...
  logging.warning(f"ecu disable {hex(addr), sub_addr} ...")
  ...
  for i in range(retry):
    try:
      ...
      for _, _ in query.get_data(timeout).items():
        logging.warning("communication control disable tx/rx ...")

        query = IsoTpParallelQuery(sendcan, logcan, bus, [(addr, sub_addr)], [com_cont_req], [COM_CONT_RESPONSE], debug=debug)
        query.get_data(0)

        logging.warning("ecu disabled")
        return True

    except Exception:
      logging.exception("ecu disable exception")

    logging.error(f"ecu disable retry ({i + 1}) ...")
  logging.error("ecu disable failed")
  return False
