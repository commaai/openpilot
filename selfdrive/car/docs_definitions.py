...
class CarInfo:
  ...
  def init(self, CP: car.CarParams, all_footnotes: Dict[Enum, int]):
    ...
    if self.harness is not Harness.none:
      ...
      harness_col = f'<img width=500000/><details><summary>Content</summary><br>{harness_col}<br><sub>' + '<br><sub>'.join([x.value + '</sub>' for x in self.harness_kit_content]) + '</details>'
