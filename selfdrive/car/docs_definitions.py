...
class CarInfo:
  ...
  def init(self, CP: car.CarParams, all_footnotes: Dict[Enum, int]):
    ...
    if self.harness is not Harness.none:
      ...
      harness_col = f'<details style="width: 135px;text-align: left; font-size: 15px;"><summary>Content</summary><br>{harness_col}<br>' + '<br>'.join([x.value for x in self.harness_kit_content]) + '</details>'
