...
class Kit(EnumBase):
  red_panda_kit = BasePart("CAN FD panda kit", parts=[Device.red_panda, Accessory.harness_box, Cable.usb_a_2_a_cable, \
                                                      Cable.usbc_otg_cable, Cable.obd_c_cable_1_5ft])
...
class CommonFootnote(Enum):
  EXP_LONG_AVAIL = CarFootnote(
    "Experimental openpilot longitudinal control is available behind a toggle; the toggle is only available in \
      non-release branches such as `devel` or `master-ci`. ",
