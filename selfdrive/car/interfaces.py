...
from openpilot.selfdrive.car import apply_hysteresis, gen_empty_fingerprint, scale_rot_inertia, scale_tire_stiffness, \
  STD_CARGO_KG
...
class CarControllerBase(ABC, Generic[CST]):
  def __init__(self, dbc_name, CP: car.CarParams, VM: VehicleModel):
    ...
    self.VM = VM
    try:
      CarControllerParams = importlib.import_module(f'openpilot.selfdrive.car.{CP.carName}.values').CarControllerParams
