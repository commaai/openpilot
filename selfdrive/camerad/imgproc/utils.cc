...
const int NUM_SEGMENTS_X = 8;
const int NUM_SEGMENTS_Y = 6;
const int LM_THRESH = 120;
const float LM_PREC_THRESH = 0.9;  // 90 perc is blur

const int FULL_STRIDE_X = 1280;
// const int FULL_STRIDE_Y = 896;
const int CONV_LOCAL_WORKSIZE = 16;
...
static uint16_t get_lapmap_one(const int16_t *lap, int x_pitch, int y_pitch) {
...
Rgb2Gray::Rgb2Gray(cl_device_id device_id, cl_context ctx, int rgb_width, int rgb_height, int filter_size)
  ...
  snprintf(args, sizeof(args),
           ...
           width, height, 1,
           filter_size, filter_size / 2, (filter_size / 2) * 2, (filter_size / 2) * width);
  prg = cl_program_from_file(ctx, device_id, "imgproc/conv.cl", args);
