...
inline void convert_uv(__global uchar * out_yuv, int uvi,
  ...
  if(uvi >= RGB_SIZE  + RGB_SIZE / 2)
    printf("UV overflow, %d >= %d\n", ui, RGB_SIZE  + RGB_SIZE / 2);
#endif
  uchar2 uv = (uchar2)(
    RGB_TO_U(ar, ag, ab),
    RGB_TO_V(ar, ag, ab)
  );
  vstore2(uv, 0, out_yuv + uvi);
}
...
#ifdef CL_DEBUG1
  if(uvi > RGB_SIZE  + RGB_SIZE / 2 - 2)
    printf("U 2 overflow, %d >= %d\n", ui, RGB_SIZE  + RGB_SIZE / 2 - 2);
#endif
  vstore4(uv2, 0, out_yuv + uvi);
}
                    ...
                    __global uchar * out_yuv)
{
  ...
  int uvi = mad24(row / 2, UV_WIDTH, RGB_SIZE + col);
  ...
  if(num_row == 4) {
    ...
    if(num_col == 4) {
      ...
      convert_2_uvs(out_yuv, uvi + UV_WIDTH, rgbs2_0, rgbs3_0, rgbs2_1, rgbs3_1);
    } else if(num_col == 2) {
      ...
      convert_uv(out_yuv, uvi + UV_WIDTH, rgbs2_0, rgbs3_0);
    }
  } else {
    const uchar8 rgbs0_0 = vload8(0, rgb + bgri_start);
    const uchar8 rgbs0_1 = vload8(0, rgb + bgri_start + 8);
