...
inline half val_from_10(const uchar * const source, int gx, int gy, half black_level) {
  // parse 12bit
  const int start = gy * FRAME_STRIDE + (3 * (gx / 2));
  const int offset = gx % 2;
  const uint major = (uint)source[start + offset] << 4;
  const uint minor = (source[start + 2] >> (4 * offset)) & 0xf;
...
__kernel void debayer10(__global const uchar * const in, __global uchar * out, __local half * cached, float black_level) {
  ...
  if (lid_x <= 1) {
    ...
    cached[mad24(y_local, localRowLen, x_local + 4)] = val_from_10(in, x_global + 4, y_global, black_level);
  }
  ...
  if (lid_x == 0 && lid_y == 0) {
    cached[0] = val_from_10(in, x_global - 1, y_global - 1, black_level);
  } else if (lid_x == get_local_size(0) - 1 && lid_y == 0) {
    cached[localRowLen - 1] = val_from_10(in, x_global + 4, y_global - 2, black_level);
  } else if (lid_x == 0 && lid_y == get_local_size(1) - 1) {
    cached[mul24(localColLen - 1, localRowLen)] = val_from_10(in, x_global - 1, y_global + 2, black_level);
  } else if (lid_x == get_local_size(0) - 1 && lid_y == get_local_size(1) - 1) {
    cached[mad24(localColLen, localRowLen, -1)] = val_from_10(in, x_global + 4, y_global + 1, black_level);
  }
