...
static void camera_init(VisionIpcServer * v, CameraState *s, int camera_id, int camera_num, unsigned int fps, VisionStreamType rgb_type, VisionStreamType yuv_type) {
...
void cameras_init(VisionIpcServer *v, MultiCameraState *s) {
  camera_init(v, &s->rear, CAMERA_ID_AR0231, 1, 20, VISION_STREAM_RGB_BACK, VISION_STREAM_YUV_BACK); // swap left/right
  printf("rear initted \n");
  camera_init(v, &s->wide, CAMERA_ID_AR0231, 0, 20, VISION_STREAM_RGB_WIDE, VISION_STREAM_YUV_WIDE);
  printf("wide initted \n");
  camera_init(v, &s->front, CAMERA_ID_AR0231, 2, 20, VISION_STREAM_RGB_FRONT, VISION_STREAM_YUV_FRONT);
