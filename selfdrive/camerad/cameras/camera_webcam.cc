...
#define ROAD_CAMERA_ID 1
#define DRIVER_CAMERA_ID 2
...
void run_camera(CameraState *s, cv::VideoCapture &video_cap, float *ts) {
  ...
  while (!do_exit) {
    ...
    CL_CHECK(clEnqueueReadBuffer(buf.copy_q, buf.buf_cl, CL_TRUE, 0, transformed_size, transformed_mat.data, 0, NULL, NULL));
...
static void road_camera_thread(CameraState *s) {
  ...
  cv::VideoCapture cap_road(ROAD_CAMERA_ID); // road
...
void driver_camera_thread(CameraState *s) {
  cv::VideoCapture cap_driver(DRIVER_CAMERA_ID); // driver
