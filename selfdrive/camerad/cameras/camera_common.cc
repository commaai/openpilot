...
public:
  ...
  void queue(cl_command_queue q, cl_mem cam_buf_cl, cl_mem buf_cl, int width, int height, float gain) {
    ...
    if (Hardware::TICI()) {
      ...
      CL_CHECK(clSetKernelArg(krnl_, 3, sizeof(int), &ggain));
...
#ifdef QCOM2
    float gain = camera_state->analog_gain + 4 * camera_state->dc_gain_enabled;
#else
    float gain = camera_state->digital_gain;
