...
void PandaPigeon::set_baud(int baud) {
  panda->hw_control_write(0xe2, 1, 0);
  panda->hw_control_write(0xe4, 1, baud/300);
}
...
std::string PandaPigeon::receive() {
  ...
  while (r.length() < 0x1000) {
    int len = panda->hw_control_read(0xe0, 1, 0, dat, sizeof(dat));
    if (len <= 0) break;
...
void PandaPigeon::set_power(bool power) {
  panda->hw_control_write(0xd9, power, 0);
}
