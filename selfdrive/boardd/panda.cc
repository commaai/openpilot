...
const int USB_MAX_TRANSFER_RETRIES = 10;
...
void Panda::handle_usb_issue(int err, int retries, const char func[]) {
  LOGE_100("usb error %d \"%s\" in %s. retries: %d", err, libusb_strerror((enum libusb_error)err), func, retries);
