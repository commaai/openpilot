...
int Panda::hw_control_write(uint8_t bRequest, uint16_t wValue, uint16_t wIndex, unsigned int timeout) {
  int err;
...
int Panda::hw_control_read(uint8_t bRequest, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength, unsigned int timeout) {
  int err;
...
std::optional<can_health_t> Panda::get_can_state(uint16_t can_number) {
  ...
  int err = hw_control_read(0xc2, can_number, 0, (unsigned char*)&can_health, sizeof(can_health));
...
void Panda::set_canfd_non_iso(uint16_t bus, bool non_iso) {
  hw_control_write(0xfc, bus, non_iso);
}
