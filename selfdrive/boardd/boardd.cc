...
void *safety_setter_thread(void *s) {
  ...
  usb_write(0xdc, (uint16_t)(cereal::CarParams::SafetyModel::ELM327), 0, &usb_lock);
  ...
  usb_write(0xdc, (uint16_t)(cereal::CarParams::SafetyModel::NO_OUTPUT), 0, &usb_lock);
  ...
  usb_write(0xdc, safety_model, safety_param);
...
bool usb_connect() {
  ...
  // get panda fw
  err = usb_read(0xd3, 0, 0, fw_sig_buf, 64);
  err2 = usb_read(0xd4, 0, 0, fw_sig_buf + 64, 64);
  if ((err == 64) && (err2 == 64)) {
  ...
  // get panda serial
  err = usb_read(0xd0, 0, 0, serial_buf, 16);
...
#ifndef __x86_64__
  if (!connected_once) {
    usb_write(0xe6, (uint16_t)(cereal::HealthData::UsbPowerMode::CDP), 0);
  }
#endif
  connected_once = true;
...
void can_health(PubMaster &pm) {
  ...
  // recv from board
  if (dev_handle != NULL) {
    cnt = usb_read(0xd2, 0, 0, (unsigned char*)&health, sizeof(health), &usb_lock);
  ...
  if ((no_ignition_exp || (voltage_f < VBATT_PAUSE_CHARGING)) && cdp_mode && !ignition) {
    ...
    if (disable_power_down.size() != 1 || disable_power_down[0] != '1') {
      ...
      usb_write(0xe6, (uint16_t)(cereal::HealthData::UsbPowerMode::CLIENT), 0, &usb_lock);
  ...
  usb_read(0xb2, 0, 0, (unsigned char*)&fan_speed_rpm, sizeof(fan_speed_rpm), &usb_lock);
  ...
  while (!do_exit) {
    ...
    if (sm.updated("thermal")){
      ...
      if (fan_speed != prev_fan_speed || cnt % 100 == 0){
        usb_write(0xb1, fan_speed, 0, &usb_lock);
    ...
    if (ir_pwr != prev_ir_pwr || cnt % 100 == 0 || ir_pwr >= 50.0){
      usb_write(0xb0, ir_pwr, 0, &usb_lock);
      prev_ir_pwr = ir_pwr;
    }
...
void pigeon_set_power(int power) {
  pthread_mutex_lock(&usb_lock);
  int err = usb_write(0xd9, power, 0);
  if (err < 0) { handle_usb_issue(err, __func__); }
  pthread_mutex_unlock(&usb_lock);
}
...
void pigeon_set_baud(int baud) {
  int err;
  pthread_mutex_lock(&usb_lock);
  err = usb_write(0xe2, 1, 0);
  if (err < 0) { handle_usb_issue(err, __func__); }
  err = usb_write(0xe4, 1, baud/300);
  if (err < 0) { handle_usb_issue(err, __func__); }
  pthread_mutex_unlock(&usb_lock);
}
...
void *pigeon_thread(void *crap) {
  ...
  while (!do_exit) {
    ...
    while (alen < 0xfc0) {
      pthread_mutex_lock(&usb_lock);
      int len = usb_read(0xe0, 1, 0, dat+alen, 0x40);
