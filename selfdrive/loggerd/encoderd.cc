...
void logger_rotate(LoggerdState *s) {
  {
    ...
    assert(err == 0);*/
    s->logger.part++;
    snprintf(s->segment_path, sizeof(s->segment_path), "fake/%d", s->logger.part);
    s->rotate_segment = s->logger.part;
    s->ready_to_rotate = 0;
    s->last_rotate_tms = millis_since_boot();
  }
  s->rotate_cv.notify_all();
  LOGW((s->logger.part == 0) ? "encoding to %s" : "rotated to %s", s->segment_path);
}
