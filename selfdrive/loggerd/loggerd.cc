...
public:
  ...
  RotateState(SubSocket *sock, const LogCameraInfo &camera_info) : fpkt_sock(sock), ci(camera_info), stream_frame_id(0), log_frame_id(0),
                  last_rotate_frame_id(UINT32_MAX), should_rotate(false), rotating(false), cur_seg(-1) {};
...
void encoder_thread(RotateState *r) {

  RotateState &rotate_state = *r;
  LogCameraInfo &cam_info = rotate_state.ci;
    ...
    while (!do_exit) {
      ...
      // all the rotation stuff
      {
        ...
        if (rotate_state.should_rotate) {
          ...
          if (rotate_state.last_rotate_frame_id == UINT32_MAX) {
...
int main(int argc, char** argv) {
  ...
  for (const auto& it : services) {
    ...
    if (camera != cameras_logged.end() && (camera->second.id != D_CAMERA || record_front)) {
      RotateState * rotate_state = new RotateState(sock, camera->second);
      s.rotate_state.push_back(rotate_state);
      encoder_threads.push_back(std::thread(encoder_thread, rotate_state));
    }
  ...
  while (!do_exit) {
    ...
    for (auto sock : poller->poll(1000)) {
      ...
      if (last_msg) {
        auto it = std::find_if(s.rotate_state.begin(), s.rotate_state.end(), [&](auto r) { return sock == r->fpkt_sock; });
        if (it != s.rotate_state.end()) {
          ...
          RotateState *r = *it;
          if (r->ci.id == F_CAMERA) {
            r->setLogFrameId(event.getFrame().getFrameId());
          } else if (r->ci.id == D_CAMERA) {
            r->setLogFrameId(event.getFrontFrame().getFrameId());
          } else if (r->ci.id == E_CAMERA) {
            r->setLogFrameId(event.getWideFrame().getFrameId());
          }
    ...
    if (s.logger.part > -1) {
      ...
      if (tms - last_camera_seen_tms <= NO_CAMERA_PATIENCE && encoder_threads.size() > 0) {
        ...
        for (auto &r : s.rotate_state) {
          ...
          new_segment &= (r->stream_frame_id >= (r->last_rotate_frame_id + SEGMENT_LENGTH * MAIN_FPS) && !r->should_rotate);
#ifndef QCOM2
