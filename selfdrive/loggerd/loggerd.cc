  ...
  while (!do_exit) {
    ...
    if (encoders.empty()) {
      ...
      if (cam_info.has_qcamera) {
        encoders.push_back(new Encoder(qcam_info.filename, buf_info.width, buf_info.height,
    ...
    while (!do_exit) {
      ...
      for (int i = 0; i < encoders.size(); ++i) {
        int out_id = encoders[i]->encode_frame(buf->y, buf->u, buf->v, extra.timestamp_eof);
        if (out_id == -1) {
