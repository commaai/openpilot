...
class TestAthenadMethods(unittest.TestCase):
  ...
  def test_do_upload(self, host):
    ...
    # ct = ClassTracker()
    # ct.track_class(athenad)
    # ct.track_object(athenad)
    # ct.track_object(athenad)
    # ct.create_snapshot()
    # ct.create_snapshot()
    # ct.create_snapshot(compute_total=True)
    # ob = ObjectTracker()
    for _ in range(5):
      tracker.print_diff()
      print()
    # before = tracker.create_summary()
    # tr = SummaryTracker()
    # tr.diff()
    # a = ['x'] * 10000
    # tr.print_diff()
    # del a
    # after = tracker.create_summary()
    # tracker.print_diff(before, after)
    # tracker.store_summary('before')
    ...
    # tracker.print_diff()
    # after = tracker.create_summary()
    # # tracker.store_summary('after')
    # print(list(tracker.format_diff(before, after))[0])
