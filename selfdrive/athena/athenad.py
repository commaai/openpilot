  ...
  while not end_event.is_set():
    ...
    try:
      ...
      try:
        ...
        cloudlog.event("athena.upload_handler.upload_start", fn=fn, sz=sz, network_type=sm['deviceState'].networkType)
        response = _do_upload(cur_upload_items[tid], cb)

        if response.status_code not in (200, 201, 403, 412):
          cloudlog.event("athena.upload_handler.retry", status_code=response.status_code, fn=fn, sz=sz, network_type=sm['deviceState'].networkType)
          retry_upload(tid, end_event)
        else:
          cloudlog.event("athena.upload_handler.success", fn=fn, sz=sz, network_type=sm['deviceState'].networkType)

        UploadQueueCache.cache(upload_queue)
      except (requests.exceptions.Timeout, requests.exceptions.ConnectionError, requests.exceptions.SSLError):
        cloudlog.event("athena.upload_handler.timeout", fn=fn, sz=sz, network_type=sm['deviceState'].networkType)
        retry_upload(tid, end_event)
      except AbortTransferException:
        cloudlog.event("athena.upload_handler.abort", fn=fn, sz=sz, network_type=sm['deviceState'].networkType)
        retry_upload(tid, end_event, False)
