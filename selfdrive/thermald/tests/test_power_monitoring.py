...
class TestPowerMonitoring(unittest.TestCase):
  ...
  @parameterized.expand([(log.HealthData.HwType.whitePanda,),
  ...
  def test_offroad_integration_white(self, hw_type):
    with patch("selfdrive.thermald.power_monitoring.get_battery_voltage", return_value=4e6):
      with patch("selfdrive.thermald.power_monitoring.get_battery_current", return_value=1e5):
        with patch("selfdrive.thermald.power_monitoring.get_battery_status", return_value="Charging"):
          pm = PowerMonitoring()
  ...
  def test_offroad_integration_discharging(self, hw_type):
    BATT_VOLTAGE = 4
    BATT_CURRENT = 1
    with patch("selfdrive.thermald.power_monitoring.get_battery_voltage", return_value=BATT_VOLTAGE * 1e6):
      with patch("selfdrive.thermald.power_monitoring.get_battery_current", return_value=BATT_CURRENT * 1e6):
        with patch("selfdrive.thermald.power_monitoring.get_battery_status", return_value="Discharging"):
          pm = PowerMonitoring()
          for _ in range(TEST_DURATION_S + 1):
  ...
  def test_max_time_offroad(self, hw_type):
    ...
    with patch("selfdrive.thermald.power_monitoring.get_battery_voltage", return_value=BATT_VOLTAGE * 1e6):
      with patch("selfdrive.thermald.power_monitoring.get_battery_current", return_value=BATT_CURRENT * 1e6):
        with patch("selfdrive.thermald.power_monitoring.get_battery_status", return_value="Discharging"):
          with patch("selfdrive.thermald.power_monitoring.MAX_TIME_OFFROAD_S", MOCKED_MAX_OFFROAD_TIME):
            pm = PowerMonitoring()
            pm.car_battery_capacity_uWh = CAR_BATTERY_CAPACITY_uWh
            start_time = ssb
