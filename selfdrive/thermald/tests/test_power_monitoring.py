...
class TestPowerMonitoring(unittest.TestCase):
  ...
  def test_car_voltage(self):
    ...
    with pm_patch("VOLTAGE_SHUTDOWN_MIN_OFFROAD_TIME_S",
                  VOLTAGE_SHUTDOWN_MIN_OFFROAD_TIME_S, constant=True), pm_patch("HARDWARE.get_current_power_draw", POWER_DRAW):
      pm = PowerMonitoring()
      pm.car_battery_capacity_uWh = CAR_BATTERY_CAPACITY_uWh
      ignition = False
      start_time = ssb
