  ...
  def handle_log(self, t, which, msg):
    ...
    # calculate lateral accel from past steering torque
      ...
      if len(self.raw_points['steer_torque']) == self.hist_len:
        ...
        # add lag to check up to now
        lat_active = np.interp(np.arange(t - MIN_ENGAGE_BUFFER, t + self.lag, DT_MDL), self.raw_points['carControl_t'], self.raw_points['lat_active']).astype(bool)
        steer_override = np.interp(np.arange(t - MIN_ENGAGE_BUFFER, t + self.lag, DT_MDL), self.raw_points['carState_t'], self.raw_points['steer_override']).astype(bool)
