...
class CarKalman(KalmanFilter):
  ...
  def __init__(self, generated_dir, steer_ratio=15, stiffness_factor=1, angle_offset=0, P_initial=None):  # pylint: disable=super-init-not-called
    ...
    # init filter
    self.filter = EKF_sym_pyx(generated_dir, self.name, self.Q, self.initial_x, self.P_initial, dim_state, \
                              dim_state_err, global_vars=self.global_vars, logger=cloudlog)
