...
void localizer_handle_msg_bytes(Localizer_t localizer, const char* service, const char *data, size_t size);'''
  ...
  def localizer_handle_msg(self, msg_name, msg_builder):
    bytstr = msg_builder.to_bytes()
    self.lib.localizer_handle_msg_bytes(self.localizer, self.ffi.new("const char[]", msg_name.encode('ascii')), self.ffi.from_buffer(bytstr), len(bytstr))
  ...
  def test_liblocalizer(self):
    msg_name = "liveCalibration"
    msg = messaging.new_message(msg_name)
    msg.liveCalibration.validBlocks = random.randint(1, 10)
    msg.liveCalibration.rpyCalib = [random.random() / 10 for _ in range(3)]

    self.localizer_handle_msg(msg_name, msg)
