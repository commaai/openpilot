...
def check_excessive_actuation(sm: messaging.SubMaster, CS: car.CarState, calibrated_pose: Pose, excessive_counter: int, engaged_counter) -> tuple[int, int, bool]:
  ...
  engaged_counter = engaged_counter + 1 if sm['carControl'].latActive and not CS.steeringPressed else 0
  if engaged_counter > MIN_ENGAGE_BUFFER:
  ...
  excessive_counter = excessive_counter + 1 if livepose_valid and (excessive_long_actuation or excessive_lat_actuation) else 0

  # if counter > 0:
  #   print('counter', counter, excessive_long_actuation, excessive_lat_actuation, livepose_valid)

  return excessive_counter, engaged_counter, excessive_counter > MIN_EXCESSIVE_ACTUATION_COUNT
