...
QString WifiManager::get_ipv4_address() {
  ...
  for (const auto &p : get_active_connections()) {
    auto pth = call<QDBusObjectPath>(p.path(), NM_DBUS_INTERFACE_PROPERTIES, "Get", NM_DBUS_INTERFACE_ACTIVE_CONNECTION, "Ip4Config");
    ...
    if (type == "802-11-wireless") {
      ...
      const auto &arr = call<QDBusArgument>(ip4config, NM_DBUS_INTERFACE_PROPERTIES, "Get", NM_DBUS_INTERFACE_IP4_CONFIG, "AddressData");
