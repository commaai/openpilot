...
void Networking::connectToNetwork(const Network &n) {
  if (wifi->isKnownConnection(n.ssid)) {
    ...
    QString pass = InputDialog::getText("Enter password for \"" + n.ssid + "\"", 8, this);
    if (!pass.isEmpty()) {
      wifi->connect(n, pass);
    }
  }
...
void Networking::wrongPassword(const QString &ssid) {
  for (Network n : wifi->seen_networks) {
    if (n.ssid == ssid) {
      QString pass = InputDialog::getText("Wrong password for \"" + n.ssid +"\"", 8, this);
      if (!pass.isEmpty()) {
        wifi->connect(n, pass);
      }
      return;
...
AdvancedNetworking::AdvancedNetworking(QWidget* parent, WifiManager* wifi): QWidget(parent), wifi(wifi) {
  ...
  connect(editPasswordButton, &ButtonControl::released, [=]() {
    QString pass = InputDialog::getText("Enter new tethering password", 8, wifi->getTetheringPassword(), this);
    if (!pass.isEmpty()) {
