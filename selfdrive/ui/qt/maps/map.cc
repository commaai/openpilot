...
void MapWindow::initLayers() {
  ...
  if (!m_map->layerExists("navLayer")) {
    ...
    m_map->setPaintProperty("navLayer", "line-width", uiState()->scene.navigate_on_openpilot ? 8.75 : 7.25);
    m_map->setPaintProperty("navLayer", "line-width-transition", transition);
...
//    bool nav_enabled = Params().getBool("ExperimentalMode");
    if (nav_enabled != uiState()->scene.navigate_on_openpilot) {
      ...
      if (loaded_once) {
        ...
        m_map->setPaintProperty("navLayer", "line-width", nav_enabled ? 8.75 : 7.25);
      }
      if (nav_enabled) {
