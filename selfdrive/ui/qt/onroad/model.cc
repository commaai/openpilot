...
void ModelRenderer::draw(QPainter &painter, const QRect &surface_rect) {
  ...
  experimental_model = sm["controlsState"].getControlsState().getExperimentalMode();
...
void ModelRenderer::drawLaneLines(QPainter &painter, const cereal::ModelDataV2::Reader &model, float max_dist) {
  ...
  for (int i = 0; i < lines.size(); ++i) {
    mapLineToPolygon(lines[i], 0.025f * probs[i], 0.0f, &polygon, max_dist);
  ...
  for (int i = 0; i < stds.size(); ++i) {
    mapLineToPolygon(edges[i], 0.025f, 0.0f, &polygon, max_dist);
    painter.setBrush(QColor::fromRgbF(1.0, 0, 0, std::clamp(1.0f - stds[i], 0.0f, 1.0f)));
    painter.drawPolygon(polygon);
  }
}
...
void ModelRenderer::drawPath(QPainter &painter, const cereal::ModelDataV2::Reader &model, float max_dist, int surface_height) {
  ...
  mapLineToPolygon(model.getPosition(), 0.9f, 1.22f, &path_polygon, max_dist, false);
  ...
  if (experimental_model) {
    ...
    for (int i = 0; i < max_len; ++i) {
      ...
      float y_pos = path_polygon[track_idx].y();

      if (y_pos < 0 || y_pos > surface_height) continue;

      // Flip so 0 is bottom of frame
      float lin_grad_point = (surface_height - y_pos) / surface_height;
...
void ModelRenderer::drawLead(QPainter &painter, const cereal::RadarState::LeadData::Reader &lead_data,
  ...
  int z_index = 0;
  for (auto x : model_position.getX()) {
    if (x > d_rel) break;
    ++z_index;
  }
  auto lead_pos = mapToScreen(d_rel, -lead_data.getYRel(), model_position.getZ()[z_index] + 1.22f);

  float sz = std::clamp((25 * 30) / (d_rel / 3 + 30), 15.0f, 30.0f) * 2.35f;
  float x = std::clamp<float>(lead_pos.x(), 0.f, surface_rect.width() - sz / 2.0f);
  float y = std::min<float>(lead_pos.y(), surface_rect.height() - sz * 0.6f);
  ...
  float alpha = calculateLeadAlpha(d_rel, lead_data.getVRel());
...
QPointF ModelRenderer::mapToScreen(float in_x, float in_y, float in_z) {
  const auto pt = matvecmul3(car_space_transform, (vec3){{in_x, in_y, in_z}});
  return {pt.v[0] / pt.v[2], pt.v[1] / pt.v[2]};
}

void ModelRenderer::mapLineToPolygon(const cereal::XYZTData::Reader &line, float y_off, float z_off,
                                     QPolygonF *pvd, float distance, bool allow_invert) {
  ...
  for (int i = 0; i < x_coords.size(); ++i) {
    ...
    auto left = mapToScreen(x, y - y_off, z + z_off);
    auto right = mapToScreen(x, y + y_off, z + z_off);
