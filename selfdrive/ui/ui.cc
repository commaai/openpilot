...
static void update_line_data(const UIState *s, const cereal::ModelDataV2::XYZTData::Reader &line,
                             float y_off, float z_off, line_vertices_data *pvd, const int max_idx = TRAJECTORY_SIZE - 1) {
...
static void update_model(UIState *s, const cereal::ModelDataV2::Reader &model) {
  ...
  for (int i = 0; i < std::size(scene.lane_line_vertices); i++) {
    ...
    update_line_data(s, lane_lines[i], 0.025 * scene.lane_line_probs[i], 0, &scene.lane_line_vertices[i]);
  }
  ...
  for (int i = 0; i < std::size(scene.road_edge_vertices); i++) {
    ...
    update_line_data(s, road_edges[i], 0.025, 0, &scene.road_edge_vertices[i]);
  }
