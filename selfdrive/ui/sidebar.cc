...
static void ui_draw_sidebar_network_strength(UIState *s, cereal::ThermalData::Reader& thermal) {
  ...
  const int img_idx = thermal.getNetworkType() == cereal::ThermalData::NetworkType::NONE ? 0 : network_strength_map[thermal.getNetworkStrength()];
  ui_draw_image(s->vg, network_img_x, network_img_y, network_img_w, network_img_h, s->img_network[img_idx], 1.0f);
}

static void ui_draw_sidebar_battery_icon(UIState *s, cereal::ThermalData::Reader& thermal) {
  ...
  int battery_img = thermal.getBatteryStatus() == "Charging" ? s->img_battery_charging : s->img_battery;

  ui_draw_rect(s->vg, battery_img_x + 6, battery_img_y + 5,
               ((battery_img_w - 19) * (thermal.getBatteryPercent() * 0.01)), battery_img_h - 11, COLOR_WHITE);

  ui_draw_image(s->vg, battery_img_x, battery_img_y, battery_img_w, battery_img_h, battery_img, 1.0f);
}

static void ui_draw_sidebar_network_type(UIState *s, cereal::ThermalData::Reader& thermal) {
  ...
  const char *network_type = network_type_map[thermal.getNetworkType()];
...
static void ui_draw_sidebar_temp_metric(UIState *s, cereal::ThermalData::Reader& thermal) {
  ...
  snprintf(temp_value_str, sizeof(temp_value_str), "%d", thermal.getPa0());
  ...
  ui_draw_sidebar_metric(s, temp_label_str, temp_value_str, temp_severity_map[thermal.getThermalStatus()], temp_y_offset, NULL);
}
...
void ui_draw_sidebar(UIState *s) {
  ...
  auto thermal = (*s->sm)["thermal"].getThermal();
  ui_draw_sidebar_network_strength(s, thermal);
  ui_draw_sidebar_battery_icon(s, thermal);
  ui_draw_sidebar_network_type(s, thermal);
  ui_draw_sidebar_temp_metric(s, thermal);
