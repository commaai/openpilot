...
class ExperimentalModeButton(Widget):
  ...
  def _handle_interaction(self):
    ...
    mouse_in_rect = rl.check_collision_point_rec(mouse_pos, self._rect)
  ...
  def _render(self):
    ...
    line_x = self._rect.x + self._rect.width - self.img_width - (2 * self.horizontal_padding)
    separator_color = rl.Color(0, 0, 0, 77)  # 0x4d = 77
    rl.draw_line_ex(rl.Vector2(line_x, self._rect.y), rl.Vector2(line_x, self._rect.y + self._rect.height), 3, separator_color)

    # Draw text label (left aligned)
    text = "EXPERIMENTAL MODE ON" if self.experimental_mode else "CHILL MODE ON"
    text_x = self._rect.x + self.horizontal_padding
    text_y = self._rect.y + self._rect.height / 2 - 45 // 2  # Center vertically
    ...
    icon_x = self._rect.x + self._rect.width - self.horizontal_padding - self.img_width
    icon_y = self._rect.y + (self._rect.height - self.img_width) / 2
