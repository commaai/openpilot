...
static void update_offroad_layout_state(UIState *s, PubMaster *pm) {
  ...
  if (prev_collapsed != s->sidebar_collapsed || prev_app != s->active_app || timeout == 0) {
    ...
    LOGD("setting active app to %d with sidebar %d", (int)s->active_app.load(), s->sidebar_collapsed.load());
...
int main(int argc, char* argv[]) {
  ...
  while (!do_exit) {
    ...
    ui_draw(s);
    ++frame;
    if (double ms = millis_since_boot() - u1; ms > 66) {
      // warn on sub 15fps
      LOGW("slow frame(%llu) time: %.2f", frame, ms);
    }
