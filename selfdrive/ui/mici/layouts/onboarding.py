...
class TrainingGuideDMTutorial(Widget):
  ...
  def __init__(self, continue_callback):
    ...
    # device.add_interactive_timeout_callback(self._show_bad_face_page)

    self_ref = weakref.ref(self)

    self._back_button = SmallCircleIconButton(gui_app.texture("icons_mici/setup/driver_monitoring/dm_question.png", 28, 48))
    self._back_button.set_click_callback(lambda: self._show_bad_face_page())
    ...
    self._bad_face_page = DMBadFaceDetected(HARDWARE.shutdown, lambda: self._hide_bad_face_page())
