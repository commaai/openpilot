...
class NetworkLayoutMici(NavWidget):
  ...
  def _on_network_updated(self, networks: list[Network]):
    ...
    connected_network = next((network for network in networks if network.is_connected or connecting_ssid == network.ssid), None)
    self._wifi_button.set_text(normalize_ssid(connected_network.ssid) if connected_network is not None else "wi-fi")
    self._wifi_button.set_value('connecting...' if connecting_ssid else (self._wifi_manager.ipv4_address or "not connected"))
