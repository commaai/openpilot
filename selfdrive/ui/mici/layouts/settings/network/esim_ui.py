...
def _mici_profile_name(profile: Profile) -> str:
  return f"{profile_display_name(profile)} (...{profile.iccid[-4:]})"
...
class ESimProfileButton(BigButton):
  ...
  def __init__(self, profile: Profile, cellular_manager: CellularManager):
    ...
    display_name = "comma prime" if is_comma else _mici_profile_name(profile)
    ...
    self._rename_btn = RenameButton(self._on_rename) if not is_comma else None
