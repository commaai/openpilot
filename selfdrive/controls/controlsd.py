...
class Controls:
  ...
  def update_events(self, CS):
    ...
    if CS.brakePressed and self.CP.openpilotLongitudinalControl and CS.vEgo < 0.3:
      speeds = self.sm['longitudinalPlan'].speeds
      v_future = speeds[-1] if len(speeds) > 1 else 100.0
      if v_future >= self.CP.vEgoStarting:
