...
def update_v_cruise(v_cruise_kph, buttonEvents, button_timers, enabled):
  ...
  for b in buttonEvents:
    if b.type.raw in button_timers and not b.pressed:
      if button_timers[b.type.raw] > LONG_PRESS:
        return v_cruise_kph # end long press
      button_type = b.type.raw
      break
  else:
    for k in button_timers.keys():
      if button_timers[k] and button_timers[k] % LONG_PRESS == 0:
        button_type = k
        long_press = True
        break
  ...
  if button_type:
    ...
    if long_press and v_cruise_kph % v_cruise_delta != 0: # partial interval
      v_cruise_kph = NEAREST_FUNC[button_type](v_cruise_kph / v_cruise_delta) * v_cruise_delta
    else:
      v_cruise_kph += v_cruise_delta * INTERVAL_SIGN[button_type]
