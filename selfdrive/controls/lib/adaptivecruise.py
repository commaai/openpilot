...
def compute_speed_with_leads(v_ego, angle_steers, v_pid, l1, l2, awareness_status, CP):
  # drive limits
...
class AdaptiveCruise(object):
  def __init__(self, live20):
    ...
    self.logMonoTime = 0
    self.dead = True
  def update(self, cur_time, v_ego, angle_steers, v_pid, awareness_status, CP):
    ...
    self.v_target_lead, self.a_target, self.a_pcm, self.jerk_factor = \
      compute_speed_with_leads(v_ego, angle_steers, v_pid, self.l1, self.l2, awareness_status, CP)
