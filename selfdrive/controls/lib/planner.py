...
class Planner(object):
  ...
  def update(self, CS, LaC, LoC, v_cruise_kph, force_slow_decel):
    ...
    plan_send.plan.hasrightLaneDepart = bool(self.PP.r_poly[3] > -1.5 and not CS.rightBlinker)
    plan_send.plan.hasRightLane = bool(self.PP.r_prob > 0.5)
    plan_send.plan.hasleftLaneDepart = bool(self.PP.l_poly[3] < 1.5 and not CS.leftBlinker)
