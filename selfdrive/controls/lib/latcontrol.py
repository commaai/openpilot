...
class LatControl(object):
  ...
  def update(self, active, v_ego, angle_steers, steer_override, d_poly, angle_offset, VM, PL):
    ...
    if self.last_mpc_ts < PL.last_md_ts:
      ...
      self.cur_state = calc_states_after_delay(self.cur_state, v_ego, angle_steers, curvature_factor, cur_Steer_Ratio, VM.CP.steerActuatorDelay) # + smoothing / 200)
    ...
    if v_ego < 0.3 or not active:
      ...
      if VM.CP.steerControlType == car.CarParams.SteerControlType.torque:
        steer_feedforward = apply_deadzone(self.angle_steers_des - float(angle_offset), 0.5) 
