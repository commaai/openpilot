  ...
  return parallel (
    ...
    "${channel_name} (casync)": {
      deviceStage("build casync", "tici-needs-can", [], [
        ["build openpilot ${channel_name}", "RELEASE=1 OPENPILOT_CHANNEL=${channel_name} BUILD_DIR=/data/openpilot CASYNC_DIR=/data/casync $SOURCE_DIR/release/create_casync_build.sh"],
        ["build agnos ${channel_name}", "$SOURCE_DIR/release/create_casync_agnos_release.py /data/casync"],
