...
def pcStage(String stageName, Closure body) {
  ...
  stage(stageName) {
    ...
    def dockerArgs = "--user=batman -v /tmp/comma_download_cache:/tmp/comma_download_cache -v /tmp/scons_cache:/tmp/scons_cache -e PYTHONPATH=${env.WORKSPACE} --cpus=16 --memory 16g";
