...
def accel_to_curvature(accel, speed):
  return 1 / (speed ** 2 / accel)


lrs = [
  list(MultiLogIterator(Route("54827bf84c38b14f|2023-02-22--15-16-51").log_paths(), sort_by_time=True)),
  list(MultiLogIterator(Route("54827bf84c38b14f|2023-02-24--13-18-50").log_paths()[10:100], sort_by_time=True))
]
...
for idx, lr in enumerate(lrs):
  ...
  for msg in tqdm(lr):
    if msg.which() == 'can':
      ...
      injecting.append(abs(CC.actuatorsOutput.curvature) > 0.015)
      ...
      if {maxlen} == {len(lat_actives), len(steering_presses), len(injecting)} and \
        ...
        if abs(curvatures[0] * CS.vEgo ** 2) > 0.5:
          continue
        ...
        print('current t: {} s, lr: {}'.format(round((msg.logMonoTime - start_time) * 1e-9, 3), idx))
        print('')
        lat_actives.clear()
...
all_1s_curv_rates = np.array(all_1s_curv_rates)
all_1s_curv_rates_speeds = np.array(all_1s_curv_rates_speeds)
plt.scatter(all_1s_curv_rates_speeds, abs(all_1s_curv_rates), label='real points')
plt.plot(np.linspace(5, 30, 100), accel_to_curvature(2, np.linspace(5, 30, 100)), c='orange', label='curvature for 2 m/s^2')
plt.legend()
plt.ylim(0, 0.02)
